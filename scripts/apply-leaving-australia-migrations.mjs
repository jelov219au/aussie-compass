...
const APPLY_ACK = "APPLY_LEAVING_AUSTRALIA_MIGRATIONS_WITH_CHECKOUT_OFF";
const migrations = [
  {
    path: "../docs/migrations/20260830_leaving_australia_entitlement_v1.sql",
    version: "20260830_leaving_australia_entitlement_v1",
    appliedKey: "entitlement_applied",
  },
  {
    path: "../docs/migrations/20260830_leaving_australia_first_sale_gate_v1.sql",
    version: "20260830_leaving_australia_first_sale_gate_v1",
...
function entitlementMigrationQueries(sql, statements) {
  assert.equal(statements.length, 67, "Unexpected Leaving Australia entitlement migration shape.");
...
if (apply && process.env.LEAVING_AUSTRALIA_MIGRATION_ACK !== APPLY_ACK) {
  throw new Error("The explicit Leaving Australia migration acknowledgement is missing.");
}
if (process.env.LEAVING_AUSTRALIA_PRO_PAYMENTS_ENABLED === "true") {
  throw new Error("Leaving Australia Checkout must remain off during migration.");
}
...
const preflight = await sql`
  select
    ...
    exists (
      ...
      where version = '20260830_leaving_australia_entitlement_v1'
    ) as entitlement_applied,
    exists (
      select 1 from public.schema_migrations
      where version = '20260830_leaving_australia_first_sale_gate_v1'
...
if (apply && (!state.entitlement_applied || !state.first_sale_applied)) {
  for (const migration of migrations) {
    ...
    const queries = migration.version === "20260830_leaving_australia_entitlement_v1"
...
console.log(`LEAVING_AUSTRALIA_BASE_MIGRATION=${baseReady ? "PASS" : apply ? "FAIL" : "PENDING"} cumulative_access=${cumulativeAccessReady ? "PASS" : "PENDING"} checkout=off secrets_printed=no`);
if (apply) assert.equal(baseReady, true, "Leaving Australia base migration postflight failed.");
