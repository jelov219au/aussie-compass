  ...
  try {
    for (const width of [320, 390, 768, 1440]) {
      ...
      if (width < 768) {
        ...
        assert.equal(await page.getByRole('navigation', { name: '모바일 메뉴', exact: true }).count(), 0);
        assert.equal(await menu.getAttribute('aria-expanded'), 'false');
        assert(await menu.evaluate(node => document.activeElement === node));
        assert.deepEqual(await page.locator('header a,header button').evaluateAll(nodes => nodes.filter(node => { const rect = node.getBoundingClientRect(); return rect.width > 0 && (rect.height < 44 || rect.width < 44); }).map(node => node.getAttribute('aria-label') || node.textContent)), []);
      }
      ...
      const overflow = await page.evaluate(() => ({ width: innerWidth, documentWidth: document.documentElement.scrollWidth, elements: [...document.querySelectorAll('header *, main *')].filter(node => { const rect = node.getBoundingClientRect(); return rect.width > 0 && (rect.right > innerWidth + 1 || rect.left < -1); }).map(node => ({ tag: node.tagName, class: node.className, text: node.textContent?.slice(0, 80) })) }));
      fs.writeFileSync(qaDirectory + `/eofy-public-layout-${width}.json`, JSON.stringify(overflow, null, 2));
      assert(overflow.documentWidth <= overflow.width);
      ...
      await page.evaluate(() => scrollTo(0, 0)); await page.screenshot({ path: qaDirectory + `/eofy-${live ? 'live' : 'local'}-top-${width}.png` });
