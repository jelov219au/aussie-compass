...
const applicationStorage = readFileSync(resolve("src/lib/resumeProApplicationStorage.ts"), "utf8");
...
const checks = [
  ...
  [builder.includes("현재 화면의 경력") && builder.includes("Skills {skills.length}개를 기기에 저장할 수 있을 때 이어갑니다") && builder.includes("onClickCapture={persistBeforeContinuation}") && builder.includes("if (!persistResume(resume))"), "완료 화면은 현재 초안과 실제 저장 성공을 구분하고 저장 실패 시 Pro 이동을 막아야 합니다."],
  [builder.includes("공고 표현과 현재 이력서 비교") && builder.includes("회사별 버전과 STAR 경험 저장·재사용") && builder.includes("이력서 요약·커버레터·면접 메모 묶음 저장"), "완료 CTA는 비교·재사용·실행 결과물이라는 실제 Pro 가치를 설명해야 합니다."],
  [builder.includes('href="/resume-pro?from=resume-builder-complete"') && builder.includes("surface={resumeFunnelSurfaces.builderCompletion}") && builder.includes("context={resumeFunnelContexts.resumeBuilder}"), "Builder 완료 CTA의 고정 출처와 개인정보 안전 이벤트 태그를 보존해야 합니다."],
  [builder.includes('window.location.hash === "#resume-pro-workspace-return"') && builder.includes('href="/resume-pro/workspace#resume-pro-workspace"') && builder.includes("Pro 첫 지원서 계속하기"), "유료 작업공간에서 온 고객은 구매 화면이 아니라 기존 작업공간으로 돌아가야 합니다."],
  ...
  [toolsSection.includes('href: "/resume-builder"') && toolsSection.includes("경험을 정리하고 PDF로 준비해요") && builderPage.includes("PDF·백업 내보내기") && builderPage.includes("실제 경험 다시 사용"), "홈의 무료 Builder 경로는 경험 정리·PDF 목적을 설명하고 연결된 Builder 화면에서 백업·재사용 기능을 안내해야 합니다."],
