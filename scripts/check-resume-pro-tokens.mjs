...
assert.match(activationForm, /activationLifetimeMs = 24 \* 60 \* 60 \* 1000/);
assert.match(activationForm, /createdAt: Date\.now\(\)/);
assert.match(activationForm, /ageMs >= 0[\s\S]*ageMs <= activationLifetimeMs/);
assert.match(activationForm, /terminalNotices = new Set<NoticeKind>\(\["used", "released", "refunded", "review"\]\)/);
assert.match(activationForm, /terminalNotices\.has\(initialNotice\)[\s\S]*clearStoredActivation\(\)[\s\S]*next = null/);
...
assert.match(activationForm, /terminalNotices\.has\(nextNotice\)[\s\S]*clearStoredActivation\(\)[\s\S]*setActivation\(null\)/);
...
assert.match(resumeProPage, /!requiresBuyerRecovery && !hasActiveEntitlement && <ResumeProVisitTracker/);
assert.match(resumeProPage, /const canOfferCheckout = checkoutAvailable && !requiresBuyerRecovery && !hasActiveEntitlement/);
