...
// Reviewed imported articles extend that catalog; imports do not bypass depth.
...
const expectedSlugs = [
  ...auditedSlugs,
  "used-car-inspection-report-next-steps",
  "rental-inspection-to-application-guide",
  "australia-paid-training-opening-closing-work-time",
  "australia-early-finish-minimum-engagement-guide",
  "australia-sale-item-refund-consumer-guarantees",
].sort();

export function validateArticleDepth(articles) {
  assert.deepEqual(articles.map(article => article.slug).sort(), expectedSlugs, "the complete runtime resource catalog must match the audited 35 plus the reviewed imports");
