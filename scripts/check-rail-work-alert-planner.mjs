...
const watch = readFileSync(new URL("../src/lib/railWorkWatch.ts", import.meta.url), "utf8");
...
assert.match(component, /useLocalPlan<WatchArea\[\]>\(STORAGE_KEY, \[\], parseWatchAreas, serializeWatchAreas/, "stored data must pass whole-record validation without truncation");
assert.doesNotMatch(watch, /\.slice\(/, "oversized saved records must not be silently truncated");
...
assert.match(watch, /https:\/\/www\.google\.com\/maps\/search\/\?api=1/, "map lookup must be explicit and user initiated");
assert.match(watch, /공식 공지 원문 열기/);
assert.match(watch, /시작·종료 날짜 다시 확인/);
assert.match(watch, /대체 버스·우회 경로 확인/);
assert.match(watch, /접근성·막차 영향 여부 확인/);
