...
assert.match(component, /https:\/\/www\.livetraffic\.com\//, "NSW cards must link to the official Live Traffic roadwork map");
assert.match(component, /저장한 지역명은 Live Traffic NSW에 자동 전달되지 않습니다/, "the official roadwork handoff must disclose that the saved place is not transmitted");
assert.match(component, /states: \["NSW"\]/, "the NSW roadwork confirmation must not appear on unsupported state cards");
assert.match(component, /checksForState\(area\.state\)/, "mobile progress must count only checks available for the selected state");
assert.match(component, /roadwork: item\.checks\.roadwork === true/, "older and malformed storage must normalize the new roadwork check to a boolean");
...
assert.match(component, /철도 공식 공지 원문 확인/);
assert.match(component, /NSW 도로 공사·통제 지도 확인/);
...
assert.doesNotMatch(component, /livetraffic\.com[^\n]*(?:query|place)|roadworkMap[^\n]*encodeURIComponent/, "saved place text must not be transmitted to the official roadwork map");
