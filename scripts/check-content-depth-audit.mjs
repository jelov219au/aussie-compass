...
assert.equal(slugs.length, 35, "content audit baseline must match the current resource count");
...
const articleData = articles.slice(articles.indexOf("export const articles: Article[] = ["));
const sectionCount = (articleData.match(/\{ heading:/g) ?? []).length;
const sourceCount = (articleData.match(/\{ label:/g) ?? []).length;
assert.ok(standard.includes(`리소스 글 ${slugs.length}개`) && standard.includes(`${sectionCount}개 섹션과 ${sourceCount}개 출처`), "content depth standard baseline is stale");
...
console.log(`CONTENT_DEPTH_AUDIT=PASS resources=${slugs.length} standalone-guides=17 baseline=${sectionCount}/${sourceCount}`);
