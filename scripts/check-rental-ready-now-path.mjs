...
assert.ok(workspace.includes("inspectionReceipt") && workspace.includes("방문 메모와 세부 체크 결과는 개인정보 보호를 위해 가져오지 않았습니다"), "the workspace must retain only an aggregate inspection receipt");
...
assert.ok(workspace.includes("rentalReadyNowReceiptMatches") && workspace.indexOf("const handoffAlreadyImported") < workspace.indexOf("nextWorkspace.applications.length < MAX_APPLICATIONS"), "a cleanup retry must deduplicate before applying the twenty-candidate limit");
