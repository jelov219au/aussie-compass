...
if (selectedProduct !== "resume-pro" && selectedProduct !== "rental-application-pro") {
...
const runtimeStripeRemoteBoundaryReady = preflightRemoteBoundaryReady
  ...
  && (selectedProduct === "rental-application-pro"
    ? process.env.STRIPE_RENTAL_APPLICATION_PRO_PRICE_ID?.trim().startsWith("price_")
...
console.log(`Hoju Compass 결제 출시 점검 (${selectedProduct}, ${isProduction ? "Production" : "Preview/Local"}${preflight ? ", 결제 OFF 사전감사" : ""})`);
...
if (verifyStripe && runtimeStripeRemoteBoundaryReady) {
  try {
    ...
    const price = selectedProduct === "rental-application-pro"
      ? await stripe.prices.retrieve(process.env.STRIPE_RENTAL_APPLICATION_PRO_PRICE_ID.trim(), { expand: ["product"] })
      : await stripe.prices.retrieve(getResumeProStripeProductConfig().priceId, { expand: ["product"] });
    if (selectedProduct === "rental-application-pro") {
      const product = typeof price.product === "object" && price.product && !price.product.deleted ? price.product : null;
      if (!price.active || price.currency !== "aud" || price.unit_amount !== 1490 || price.type !== "one_time"
        || price.tax_behavior !== "inclusive" || price.livemode !== isProduction || !product?.active
        || product.metadata?.product_code !== "rental_application_pro" || product.metadata?.billing_model !== "one_time") {
        throw new Error("Rental Stripe Product contract mismatch");
      }
    } else {
      assertResumeProStripeProduct(price, getResumeProStripeProductConfig(), isProduction);
    }
    stripeProductVerified = true;
    console.log(`PASS  Stripe 원격 상품·세금 계약 — ${selectedProduct === "rental-application-pro" ? "Rental 가격·메타데이터·포함세" : "Product ID, tax code, 포함세 가격"} 일치`);
