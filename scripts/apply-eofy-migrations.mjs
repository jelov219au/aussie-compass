...
const migrations = [
  {
    path: "../docs/migrations/20260830_eofy_entitlement_v1.sql",
    version: "20260830_eofy_entitlement_v1",
    appliedKey: "entitlement_applied",
  },
  {
    path: "../docs/migrations/20260830_eofy_first_sale_gate_v1.sql",
    version: "20260830_eofy_first_sale_gate_v1",
...
function entitlementMigrationQueries(sql, statements) {
  assert.equal(statements.length, 67, "Unexpected EOFY entitlement migration shape.");
...
if (apply && process.env.EOFY_MIGRATION_ACK !== APPLY_ACK) {
  throw new Error("The explicit EOFY migration acknowledgement is missing.");
}
if (process.env.EOFY_PRO_PAYMENTS_ENABLED === "true") {
  throw new Error("EOFY Checkout must remain off during migration.");
}
...
const preflight = await sql`
  select
    ...
    exists (
      ...
      where version = '20260830_eofy_entitlement_v1'
...
if (apply && (!state.entitlement_applied || !state.first_sale_applied)) {
  for (const migration of migrations) {
    ...
    const queries = migration.version === "20260830_eofy_entitlement_v1"
