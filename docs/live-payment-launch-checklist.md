...
- [ ] With both payment switches still explicitly false, prove the exact-SHA Production origin and public Production pages match that candidate. The controlled Checkout check must return HTTP 503 `checkout_unavailable` without a Checkout URL.
...
Keep this order fail-closed: keep `PAYMENTS_ENABLED=false` and `STRIPE_MANAGED_PAYMENTS_ENABLED=false`; restore every required Production environment value while payments are off; verify or apply the required database migrations in the documented order while payments are off; promote the exact approved candidate while payments are off; then run the canonical outer preflight pinned to that full SHA and the approved Neon endpoint. Enabling either payment switch is a separate later owner decision, never a deployment step. For rollback, keep or restore both switches to false and roll the application back to the prior known deployment only; do not drop or reverse the additive payment evidence migrations or delete webhook, entitlement, outbox, nonce, access, restore or first-sale evidence.
...
With `PAYMENTS_ENABLED=false`, first provision `hoju_payment_auditor` through SQL with ordinary PostgreSQL privileges and apply `docs/payment-audit-role-grants.sql`; do not create this login through Neon Console/CLI/API because those creation paths grant `neon_superuser` membership. Start a fresh PowerShell process with no project, payment, Stripe, database or Vercel variables preloaded, then run `.\scripts\run-vercel-production-payment-preflight.ps1 -ExpectedNeonEndpointId ep-curly-wave-a78bktnq -ExpectedProductionSha <full-owner-approved-sha>` after separately reconfirming that endpoint as Primary. This outer wrapper loads Vercel Production values without writing a dotenv file and requests the project Automation Bypass only in the Vercel child through a masked prompt, so it never reaches `npx` or the Vercel CLI parent. Never preload or deploy the bypass. The existing inner wrapper then requests the one-off `hoju_payment_auditor` URL, Account-Read-only Stripe restricted key and dedicated Balance-Transactions-Read accounting key only at masked prompts. It fails before remote payment/database access unless runtime, audit and accounting are three distinct `rk_live_` keys. It process-scopes the audit key and database URL for `npm run payments:check -- --preflight --strict --verify-stripe --verify-database`; only after that strict audit passes, it process-scopes the same masked accounting key and runs `npm run accounting:preflight`. Record launch evidence only when the outer `VERCEL_PRODUCTION_PREFLIGHT=PASS` and inner `FIRST_SALE_PREFLIGHT=PASS mode=live payments_off=yes keys=three-distinct-rk-live required_reads=verified checkout_create=not-exercised database=strict-pass secrets_printed=no` both appear in one uninterrupted run. Any FAIL, missing line or interruption remains `NO-GO`.
