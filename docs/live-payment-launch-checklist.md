...
- [ ] With Checkout off and a current encrypted backup confirmed, apply and verify `20260829_rental_first_sale_gate_v1` before Rental's controlled live payment. Require the eighth migration version, both Rental-aware product/amount constraints, the same-signature gate functions, no reservation in flight and unchanged effective privileges; this migration is not authorised by a deployment or payment approval alone.
...
- [ ] Immediately before Rental activation, confirm the live account again has zero open Checkout Sessions across both products and retain only the non-sensitive count result.
