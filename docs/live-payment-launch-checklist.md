...
With `PAYMENTS_ENABLED=false`, first provision `hoju_payment_auditor` through SQL with ordinary PostgreSQL privileges and apply `docs/payment-audit-role-grants.sql`; do not create this login through Neon Console/CLI/API because those creation paths grant `neon_superuser` membership. From an intentionally loaded Production operator environment, run `.\scripts\run-production-payment-preflight.ps1 -ExpectedNeonEndpointId ep-curly-wave-a78bktnq -ExpectedProductionSha <full-owner-approved-sha>` after separately reconfirming that endpoint as Primary. Load the Vercel project Automation Bypass value only into that process as `VERCEL_AUTOMATION_BYPASS_SECRET`; the wrapper sends it only to the protected exact deployment origin and removes it immediately after identity verification. Never place it on a command line, in source, a tracked environment file or an application deployment variable. Before requesting any masked payment or database credential, the wrapper requires the exact full SHA to be the successful public Production deployment. Enter the one-off `hoju_payment_auditor` URL, Account-Read-only Stripe restricted key and dedicated Balance-Transactions-Read accounting key only at the masked prompts. The wrapper fails before remote payment/database access unless runtime, audit and accounting are three distinct `rk_live_` keys and no audit or accounting secret is preloaded. It process-scopes the audit key and database URL for `npm run payments:check -- --preflight --strict --verify-stripe --verify-database`; only after that strict payment/database audit passes, it process-scopes the same masked accounting key and runs `npm run accounting:preflight`. It then removes all temporary process variables and zeroes all three prompted unmanaged plaintext buffers before emitting the final result. Record the first-sale launch gate only from the exact redacted `FIRST_SALE_PREFLIGHT=PASS mode=live payments_off=yes keys=three-distinct-rk-live required_reads=verified checkout_create=not-exercised database=strict-pass secrets_printed=no` line; the supporting `STRIPE_KEY_ROLES=PASS` line is not sufficient on its own, and any FAIL, missing final PASS or interrupted run remains `NO-GO`. Never preload or persist audit/accounting credentials in shell history, the application runtime, Preview or Production environment. The command does not print stored values, IDs or connection strings. It must use the runtime key for the remote Product and zero-open-Checkout proof, the audit key for Stripe Account/profile evidence, verify the same prompted accounting key has Balance Transactions read permission, and verify the actual `hoju_app_runtime` connection, exact `hoju_payment_auditor` identity and effective write/function denials, required migrations, named entitlement-link constraint and no reservation in flight before any separate owner decision to turn payments on. Checkout creation is deliberately not exercised while payments are off.
...
- [ ] With payments off, complete `.\scripts\run-production-payment-preflight.ps1 -ExpectedNeonEndpointId ep-curly-wave-a78bktnq -ExpectedProductionSha <full-owner-approved-sha>` and retain only its exact final `FIRST_SALE_PREFLIGHT=PASS mode=live payments_off=yes keys=three-distinct-rk-live required_reads=verified checkout_create=not-exercised database=strict-pass secrets_printed=no` result. The same run must first emit the exact-SHA Production deployment PASS. For first-customer approval, this single integrated result satisfies the Balance Transactions read check; `.\scripts\run-accounting-preflight.ps1` is only for later independent revalidation after the accounting key or its permissions change, is not a second launch prerequisite in the same approval window, and cannot replace the integrated gate.
