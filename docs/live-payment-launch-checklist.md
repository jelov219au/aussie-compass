...
With `PAYMENTS_ENABLED=false`, first provision `hoju_payment_auditor` through SQL with ordinary PostgreSQL privileges and apply `docs/payment-audit-role-grants.sql`; do not create this login through Neon Console/CLI/API because those creation paths grant `neon_superuser` membership. From an intentionally loaded Production operator environment, run `.\scripts\run-production-payment-preflight.ps1 -ExpectedNeonEndpointId ep-curly-wave-a78bktnq` after separately reconfirming that endpoint as Primary. Enter the one-off `hoju_payment_auditor` URL, Account-Read-only Stripe restricted key and dedicated Balance-Transactions-Read accounting key only at the masked prompts. The wrapper fails before remote access unless runtime, audit and accounting are three distinct `rk_live_` keys; it never assigns or uses the accounting key, whose permission still requires the separate accounting preflight. Never preload, persist or assign audit/accounting credentials in shell history, the application runtime, Preview or Production environment. The wrapper invokes `npm run payments:check -- --preflight --strict --verify-stripe --verify-database`, removes temporary process variables, and zeroes prompted unmanaged plaintext buffers. Record the key-role step only from the exact redacted `STRIPE_KEY_ROLES=PASS mode=live distinct=yes permissions=separate-preflights-required secrets_printed=no` line; any FAIL or missing line remains `NO-GO`. The command does not print stored values, IDs or connection strings. It must use the runtime key for the remote Product and zero-open-Checkout proof, the audit key for Stripe Account/profile evidence, and verify the actual `hoju_app_runtime` connection, exact `hoju_payment_auditor` identity and effective write/function denials, required migrations, named entitlement-link constraint and no reservation in flight before any separate owner decision to turn payments on.
...
- [ ] With payments off, run `.\scripts\run-accounting-preflight.ps1` and enter the dedicated live accounting key only at its masked prompt. Record PASS only when the final line is exactly `ACCOUNTING_PREFLIGHT=PASS mode=live permission=balance_transactions.read private_file_written=no`. The wrapper has no test-mode override; any FAIL, missing final PASS, test-mode key, timeout, interrupted run or unknown result remains launch `NO-GO`.
