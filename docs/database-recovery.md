...
4. Compare row counts for `payment_webhook_events`, `purchase_entitlements`, `purchase_restore_tokens`, `purchase_checkout_activations`, `purchase_access_sessions`, and `payment_operator_alert_outbox` with the backup source snapshot. Report only counts, active/expired/revoked session totals and pending/sent totals; never select full Stripe IDs, access-session hashes, claim hashes or payload data.
