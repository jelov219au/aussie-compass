...
4. Compare row counts for `payment_webhook_events`, `purchase_entitlements`, `purchase_restore_tokens`, `purchase_checkout_activations`, `purchase_access_sessions`, `purchase_restore_activations`, and `payment_operator_alert_outbox` with the backup source snapshot. Report only counts, active/expired/revoked session totals and pending/sent totals; never select full Stripe IDs, access-session hashes, restore/nonce hashes, claim hashes or payload data.
...
6. Confirm the ordered versions `20260823_first_sale_gate_charge_link_v2`, `20260823_payment_operator_alert_outbox_v1`, `20260823_checkout_activation_nonce_v1`, `20260823_purchase_access_sessions_v1`, and `20260823_restore_activation_nonce_v1`. A partial migration keeps payments off; use a reviewed additive forward-fix rather than dropping tables or deleting evidence.
