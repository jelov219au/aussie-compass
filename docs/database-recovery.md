...
4. Compare row counts for `payment_webhook_events`, `purchase_entitlements`, and `purchase_restore_tokens` with the backup source snapshot.
5. Verify every entitlement references a webhook event, restore-token hashes are unique, and duplicate Stripe event IDs are impossible.
