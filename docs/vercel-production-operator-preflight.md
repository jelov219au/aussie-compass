...
Vercel Sensitive values are non-readable after creation and are available only to the deployed runtime. The wrapper therefore never uses `vercel env run`, `vercel env pull`, a dotenv file, a token argument or a manually managed Deployment Protection bypass secret. After any Production setting change, including `STRIPE_MANAGED_PAYMENTS_ENABLED=true`, redeploy the same approved full SHA and repeat the exact-SHA payment-off deployment check; the wrapper must not be run against the pre-change deployment. It then proves the exact successful Production deployment and obtains its protected `vercel.app` origin. The protected runtime check is sent with pinned `vercel curl --deployment`; the fixed response verifies Production, exact SHA, both payment switches, the remaining payment configuration, Resume Pro Product and zero open Checkout Sessions, the runtime database role/schema/endpoint, and either SMTP transport without sending mail or the exact owner-acknowledged first-sale manual monitoring fallback.
...
Only `VERCEL_PRODUCTION_PREFLIGHT=PASS ... payments=off monitoring=<smtp|manual> ... secrets_printed=no` together with the inner canonical `FIRST_SALE_PREFLIGHT=PASS ... monitoring=<smtp|manual> ...` using the same concrete value is usable evidence. `smtp` means no-send transport verification passed; `manual` means SMTP was not run and only the explicitly owner-acknowledged manual monitoring path passed. A placeholder, mismatch, missing line, interruption or `FAIL` remains `NO-GO` and does not authorize enabling payments.
