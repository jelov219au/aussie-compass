...
- 2026-08-30: the A$9.90 Pay Evidence product contract now has a product-specific kill switch and Price ID, fail-closed Checkout, first-sale gate, signed webhook entitlement, browser-bound activation, server-tracked access release, one-time recovery, paid workspace gate and local-data purge path. Two owner-reviewed SQL migrations and an offline key-name preflight are prepared but unapplied. No Stripe, Neon, Vercel or Production state changed.
- Next launch action: with explicit owner approval, enter the external keys, apply the two Pay Evidence migrations while Checkout is off, then run sandbox and payment-off Production preflights before considering the product switch.
