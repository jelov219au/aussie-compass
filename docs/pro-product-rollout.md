...
| Rental Pack Pro | Live; first-sale monitoring | Expanded 20-candidate workspace protected by its own entitlement in Production | A$14.90 one-time Checkout active; signed-webhook entitlement, recovery, refund revocation and product-scoped first-sale gate implemented |
...
7. Verify sandbox purchase, duplicate and out-of-order webhooks, recovery-code single use, full refund and immediate access revocation.
8. Pass the exact-SHA Production runtime, Stripe, database, accounting and monitoring preflights with Checkout off and zero open Sessions. A live self-purchase/refund is not required; the paid-product testing cost boundary below governs any exceptional live test.

Checkout-off Production preparation completed on 29 August 2026 with the Rental switch set to `false`. The preserved release work was subsequently reconciled with `main` at `c2025ff` in merge commit `40d0b76`; payment, webhook, entitlement, access-session and first-sale conflicts retain `main`'s newer security architecture. After Rental contracts, current-main payment preflight contracts, TypeScript, full lint, build and secret scanning passed, the owner approved the reconciled preparation deployment. Exact source `072fe2a18b7b1349edf1ceca17e5c4bb882e22cf` was promoted as Vercel Production deployment `F1qCXD7cuD8G1cM1q5T7EYjxjJXn`, reached `Ready`, and passed public checkout-off and unauthorised-workspace checks on `hojucompass.com`. The Rental switch remains `false`; action-time approval for activation and one controlled charge/refund was later granted, subject to the newly discovered migration gate below.

The subsequent action-time approval exposed a fail-closed launch gap before any switch or payment changed: the reconciled Rental route was still Preview-only and did not use the atomic first-sale reservation. The hardened candidate now applies product-scoped reservation, idempotent Session creation, exact live Product/Price verification, active-buyer blocking and atomic gate+entitlement fulfillment to Rental. Exact source `564c941007d5f6faad8f1f04696b207846cdbcb6` was deployed checkout-off as Vercel Production deployment `C9D24ih9ZojqbAxut6ygbx8mWkqF`, reached `Ready`, and passed the public checkout-off and unauthorised-workspace checks on `hojucompass.com`.

Focused approval was subsequently granted for additive migration `20260829_rental_first_sale_gate_v1`. The corrected transaction committed on Neon Primary and all 16 postflight checks passed, including both product gates, expected constraints and overloads, ownership, grants, runtime/operator privilege boundaries and zero reserved rows. The first integrated Production preflight then failed safely before its Neon URL prompt because the existing runtime still reported the shared `PAYMENTS_ENABLED=true`; no dependency check or payment ran. Production was restored to checkout-off with `PAYMENTS_ENABLED=false`, the Rental switch remained `false`, and exact source `564c941` was redeployed as `B1n5xyFzjQM2y2oqT5n7YzRDbA5s`, which reached `Ready` on `hojucompass.com`. Production remained closed until the integrated exact-SHA audit passed with zero open Sessions.

Production activation completed on 30 August 2026 after the sandbox payment/refund flow and the exact-SHA integrated preflight passed. The final gate reported Production runtime, operator audit, Stripe key separation, accounting preflight, database/runtime schema, first-sale preflight and deployment evidence as PASS with zero open Checkout Sessions. A stale expired unpaid Rental reservation was released only after its matching live Stripe Session was verified as `expired`, `unpaid` and without a PaymentIntent; this created no charge or refund.

Exact release source `dba0b5e32e59f9a2b7c15ffd1f9b5b565436ee6a` was fast-forwarded to `main` and built as Vercel Production deployment `dpl_ApJHN6gxtHRjEw2y8Ygxq1smPpBp` (`https://aussie-compass-f7uh7w5wg-aussiecompass.vercel.app`). Vercel identified `Branch: main, Commit: dba0b5e`, generated all 122 pages, reached `Ready`, and assigned `hojucompass.com` and `www.hojucompass.com`. Production `PAYMENTS_ENABLED` and `RENTAL_APPLICATION_PRO_PAYMENTS_ENABLED` are both enabled. A post-deploy public browser check confirmed the A$14.90 offer, required consent, visible purchase action, disabled-before-consent/enabled-after-consent behaviour and zero console errors. The purchase action was not clicked, no live self-charge was made, and a subsequent read-only Stripe query still returned zero open Checkout Sessions.

The remaining launch obligation is manual first-organic-sale monitoring: verify the signed webhook, Rental entitlement, durable operator alert and customer access before treating the release as stable. If that chain fails, disable both payment switches, redeploy the same approved source, and preserve event identifiers for incident review.
