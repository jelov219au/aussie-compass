...
| Pay Evidence Pack Pro | Payment-off external foundation | A$9.90 workspace, paid access and portable case archive implemented | Live catalog/Price and migrations installed with the product switch off; exact-SHA payment-off deployment and preflight remain pending |
...
Focused approval was subsequently granted for additive migration `20260829_rental_first_sale_gate_v1`. The corrected transaction committed on Neon Primary and all 16 postflight checks passed, including both product gates, expected constraints and overloads, ownership, grants, runtime/operator privilege boundaries and zero reserved rows. The first integrated Production preflight then failed safely before its Neon URL prompt because the existing runtime still reported the shared `PAYMENTS_ENABLED=true`; no dependency check or payment ran. Production was restored to checkout-off with `PAYMENTS_ENABLED=false`, the Rental switch remained `false`, and exact source `564c941` was redeployed as `B1n5xyFzjQM2y2oqT5n7YzRDbA5s`, which reached `Ready` on `hojucompass.com`. Production remains closed until the integrated exact-SHA audit passes with zero open Sessions.
...
- Next launch action: reconcile the Pay Evidence branch with current `main`, add the product-scoped runtime preflight that leaves the operating Resume and Rental gates unchanged, then obtain explicit approval for an exact-SHA deployment with only the Pay Evidence switch off. Re-enter the now-validated operator values only in the external PowerShell window for the challenge-HMAC runtime check before any switch-on decision.
