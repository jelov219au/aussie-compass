...
| Rental Application Pack Pro | Enhanced feature and payment validation | Multi-property local workspace with reusable profile, privacy checks, message templates and PDF/TXT/JSON export | Product-scoped checkout, entitlement, recovery and refund-revocation code implemented; payment remains closed until database, Stripe and test-flow gates pass |
...
The product-isolation foundation is now implemented in code: Resume Pro and Rental Application Pack use separate product codes throughout checkout metadata, webhook grants, signed access cookies, active-entitlement lookups and restore-code consumption. The Rental checkout still fails closed unless every environment readiness gate passes. The database constraint expansion and a dedicated Stripe Price must be applied and verified before any Rental checkout test is enabled.
...
4. Verify the implemented server-only Checkout path reuses the Managed Payments and seller-detail safeguards and records the purchase-terms version.
5. Verify the implemented production workspace gate accepts only an active Rental entitlement; local development remains a free preview.
