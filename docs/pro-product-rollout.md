...
The subsequent action-time approval exposed a fail-closed launch gap before any switch or payment changed: the reconciled Rental route was still Preview-only and did not use the atomic first-sale reservation. The local candidate now applies product-scoped reservation, idempotent Session creation, exact live Product/Price verification, active-buyer blocking and atomic gate+entitlement fulfillment to Rental. Production remains closed until the additive `20260829_rental_first_sale_gate_v1` Neon migration receives separate migration approval, the hardened exact SHA is deployed checkout-off, and the integrated Production audit passes.
