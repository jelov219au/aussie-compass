...
Keep `PAYMENTS_ENABLED=false` while collecting and classifying this evidence. Collect values through an approved read-only operator session after separately proving the exact Production Source SHA and integrated first-sale preflight. Schema v2 puts that same full lowercase SHA in the private packet and requires it independently as `--expected-sha`; the verifier prints only `source_sha=exact`, never the SHA. Legacy schema v1 packets fail closed. A new deployment, setting-only redeploy or source commit invalidates this rehearsal and returns the launch sequence to the exact-SHA deployment check. Do not grant the application runtime direct table access merely to populate this file.
...
```powershell
node scripts/verify-production-payment-path-evidence.mjs --expected-sha <full-owner-approved-sha> --file <private-redacted-json>
```
...
```text
PRODUCTION_PAYMENT_PATH_EVIDENCE=PASS mode=production source_sha=exact payments_off=yes webhook=verified outbox=sent nonce=bound release=denied restore=verified identifiers_printed=no secrets_printed=no
```

Any missing, extra or inconsistent field, absent/invalid expected SHA, Production Source SHA mismatch, payment switch not explicitly off, unverified migration, unsafe identifier pattern or malformed input emits one canonical `PRODUCTION_PAYMENT_PATH_EVIDENCE=FAIL ... launch=NO-GO reason=<allowlisted_reason>` line. Treat it as launch `NO-GO`; do not request another payment from the customer.
