-- Add EOFY Pack Pro to the existing product-scoped first-sale gate.
...
begin
  ...
  then
    raise exception 'Pay Evidence first-sale gate migration requires neondb_owner on neondb';
  end if;
  ...
  if exists (
    ...
    raise exception '20260830_eofy_first_sale_gate_v1 is already applied';
  end if;
...
begin
  if p_product_code is null
    ...
    or p_expected_amount_cents is distinct from (case p_product_code
      ...
      when 'pay_evidence_pro' then 990
...
as $$
begin
  if p_action = 'grant' and p_product_code in ('resume_pro', 'rental_application_pro', 'pay_evidence_pro', 'eofy_pro') then
    raise exception 'Paid-product grant requires apply_first_sale_paid_event';
  end if;
...
begin
  if not exists (
    ...
    raise exception 'Pay Evidence first-sale gate migration postflight failed';
  end if;
end;
$$;
...
values ('20260830_eofy_first_sale_gate_v1');
