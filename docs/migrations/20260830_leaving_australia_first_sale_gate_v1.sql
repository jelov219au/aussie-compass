-- Add Leaving Australia Pack Pro to the existing product-scoped first-sale gate.
...
begin
  ...
  then
    raise exception 'Leaving Australia first-sale gate migration requires neondb_owner on neondb';
  end if;
  ...
  if exists (
    ...
    raise exception '20260830_leaving_australia_first_sale_gate_v1 is already applied';
  end if;
...
as $$
begin
  if p_action = 'grant' and p_product_code in ('resume_pro', 'rental_application_pro', 'pay_evidence_pro', 'eofy_pro', 'leaving_australia_pro') then
...
begin
  if not exists (
    ...
    raise exception 'Leaving Australia first-sale gate migration postflight failed';
  end if;
end;
$$;
...
values ('20260830_leaving_australia_first_sale_gate_v1');
