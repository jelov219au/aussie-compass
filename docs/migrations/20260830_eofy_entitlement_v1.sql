-- Add EOFY Pack Pro to the paid-product entitlement and access contract.
...
create table if not exists purchase_entitlements (
  ...
  product_code text not null check (product_code in ('resume_pro', 'rental_application_pro', 'pay_evidence_pro', 'eofy_pro')),
...
alter table public.purchase_access_sessions
  ...
  check (product_code in ('resume_pro', 'rental_application_pro', 'pay_evidence_pro', 'eofy_pro'));
...
begin
  if p_checkout_session_id is null
    ...
    or p_product_code not in ('resume_pro', 'rental_application_pro', 'pay_evidence_pro', 'eofy_pro')
...
begin
  if p_checkout_session_id is null or p_checkout_session_id !~ '^cs_(test|live)_[A-Za-z0-9]+$'
    or p_product_code is null or p_product_code not in ('resume_pro', 'rental_application_pro', 'pay_evidence_pro', 'eofy_pro') then
...
begin
  if p_entitlement_id is null or p_product_code is null
    or p_product_code not in ('resume_pro', 'rental_application_pro', 'pay_evidence_pro', 'eofy_pro') then
    raise exception 'Invalid entitlement lookup input';
  end if;
  return query select entitlement.id, entitlement.product_code, entitlement.status,
    entitlement.granted_at, entitlement.revoked_at
...
create table if not exists payment_operator_alert_outbox (
  ...
  livemode boolean not null,
  product_code text check (product_code in ('resume_pro', 'rental_application_pro', 'pay_evidence_pro', 'eofy_pro')),
...
begin
  if p_event_id is null
    ...
    or (p_product_code is not null and p_product_code not in ('resume_pro', 'rental_application_pro', 'pay_evidence_pro', 'eofy_pro'))
...
insert into schema_migrations (version)
values ('20260830_eofy_entitlement_v1')
on conflict (version) do nothing;
