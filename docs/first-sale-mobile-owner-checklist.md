...
| 3 | `HOLD` | owner laptop | 같은 exact-SHA 배포와 payments off 상태에서 통합 Production preflight를 실행한다. outer/inner PASS의 `monitoring=smtp|manual`은 같은 concrete 값이어야 하며, 정확한 최종 `FIRST_SALE_PREFLIGHT=PASS` 한 줄만 PASS다. 중단·누락·불일치·FAIL은 `HOLD`다. |
