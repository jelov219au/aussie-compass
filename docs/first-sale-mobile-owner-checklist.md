...
| --- | --- | --- | --- |
| 0 | `HOLD` | local evidence / owner | 최종 후보의 full SHA를 고정하고 `PAYMENTS_ENABLED=false`를 유지한다. `STRIPE_MANAGED_PAYMENTS_ENABLED=true` 설정 뒤 같은 approved full SHA를 재배포한 다음 Production Source SHA exact match와 payment-off Checkout HTTP 503/no URL을 다시 증명한다. 이전 SHA의 PASS나 설정 반영 전 배포는 재사용하지 않는다. |
...
| 3 | `HOLD` | owner laptop | 같은 exact-SHA 배포와 payments off 상태에서 통합 Production preflight를 실행한다. outer/inner PASS의 `monitoring=smtp|manual`은 같은 concrete 값이어야 하며, 정확한 최종 `FIRST_SALE_PREFLIGHT=PASS` 한 줄만 PASS다. `monitoring=manual`은 이 runtime preflight에서만 SMTP transport를 생략할 수 있다는 뜻이며 아래 mailbox gate를 대체하지 않는다. 중단·누락·불일치·FAIL은 `HOLD`다. |
| 4 | `HOLD` | mailbox owner / owner laptop | preflight가 `monitoring=smtp` 또는 `monitoring=manual` 중 어느 쪽이든, 최종 live-sale 승인 전 별도 승인된 labelled non-customer test가 실제 mailbox에 수신됐다는 증거를 확인한다. 설정 존재나 send 결과만으로 수신 PASS를 만들지 않으며, `monitoring=manual`은 actual mailbox receipt evidence를 면제하지 않는다. |
| 5 | `HOLD` | technical owner / owner laptop | 배포를 바꾸지 않고 같은 approved full SHA와 payments off 상태에서 승인된 Production rehearsal을 수행한다. private packet과 독립 `--expected-sha`가 exact match한 `PRODUCTION_PAYMENT_PATH_EVIDENCE=PASS mode=production source_sha=exact`만 인정한다. Preview rehearsal, Production zero-row, 이전 SHA의 PASS는 대체 증거가 아니다. |
