...
Resume Pro is a one-time AUD 19.90 product provided by Hoju Compass's Australian sole-trader operator through Stripe Managed Payments. Hoju Compass's product-provider identity and the transaction seller (Merchant of Record) shown by Managed Payments are separate roles. A controlled live purchase, access-delivery check and full refund passed on 20 August 2026, but that historical test does not prove current launch readiness. The authoritative 24 August Production audit is `NO-GO`. Keep `PAYMENTS_ENABLED=false` throughout setup and payment-off preflight. Use `STRIPE_MANAGED_PAYMENTS_ENABLED=false` only for the initial payment-off deployment baseline and rollback; before the executable preflight, set it to `true` and redeploy the same approved SHA while `PAYMENTS_ENABLED` stays `false`. Neither state authorizes a sale, and the owner must separately approve one first-customer sale after every remaining gate passes.
...
- Require `operatorMonitoringConfigured=true` in Production. `monitoring=smtp` requires `operatorAlertsConfigured=true`, so `paymentAlertsConfigured()` must accept the enabled SMTP authentication and port settings. `monitoring=manual` may satisfy only the protected payment-off runtime preflight through the exact owner-acknowledged fallback; it does not claim SMTP readiness and does not waive the separate actual mailbox receipt gate before live-sale approval.
...
- Before Rental's controlled live payment, apply `docs/migrations/20260829_rental_first_sale_gate_v1.sql` while both payment switches remain off. The runtime and operator audits require this eighth migration and verify that both first-sale tables accept only the exact Resume A$19.90 and Rental A$14.90 product contracts. A missing version, constraint or replaced gate function keeps Rental Checkout closed.
...
Production runtime readiness requires `operatorMonitoringConfigured=true`. The SMTP path gets that state only when `paymentAlertsConfigured()` returns true and sets `operatorAlertsConfigured=true`; the exact owner-acknowledged manual path may set `firstSaleMonitoredModeConfigured=true` instead, solely for the protected payment-off runtime preflight. This readiness check confirms one monitoring path only. Before accepting a customer payment, regardless of the selected preflight mode, separately prove the outbox pending/sent/attempt counts, two-worker busy/stale-lease behavior and actual controlled purchase/refund delivery to the monitored mailbox without exposing sensitive payment data. Also prove activation response-loss recovery uses the same server access session, release-response loss denies the old cookie, another device remains active, and refund/review/expiry/legacy-cookie/session mismatch all fail closed.
...
1. Run the target-environment launch audit with `.\scripts\run-vercel-production-payment-preflight.ps1 -ExpectedNeonEndpointId ep-curly-wave-a78bktnq -ExpectedProductionSha <full-owner-approved-sha>` while Checkout is off and after any payment-setting or deployment change. A setting change first requires redeploying the same approved SHA and repeating the exact-SHA payment-off deployment check. Reconfirm that this non-secret ID is still the separately approved Primary compute before each run. The runtime restricted key requires Prices Read, Products Read, Checkout Sessions Write and Payment Intents Read; the separate audit and accounting keys are entered only at masked prompts. Matching outer and inner canonical PASS lines with one concrete `monitoring=smtp` or `monitoring=manual` value are necessary but not sufficient for sale approval. Keep the same deployment and `PAYMENTS_ENABLED=false`; regardless of preflight mode, obtain the separate actual mailbox receipt evidence and complete the Production rehearsal whose canonical result includes `source_sha=exact`, then let the business owner review every remaining pre-payment gate and separately approve one sale. Any intervening deployment or SHA mismatch restarts the exact-SHA sequence; preflight alone never authorizes `PAYMENTS_ENABLED=true`.
