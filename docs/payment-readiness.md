...
- After least-privilege roles v1, apply `docs/migrations/20260824_entitlement_link_conflict_v1.sql`. It is a required forward fix for the paid-event PaymentIntent/Charge link conflict target; SQLSTATE `42702` or a missing version keeps live Checkout closed.
...
- Run `npm run payments:check -- --preflight --strict --verify-stripe --verify-database` inside the target deployment environment while `PAYMENTS_ENABLED=false`. Inject `PAYMENTS_AUDIT_DB_URL` only for this command using a separately approved read-only audit credential; do not use that credential in the app runtime. The remote checks retrieve the configured Price with its Product, require zero existing open Checkout Sessions, prove the real app connection resolves to `hoju_app_runtime`, and use the separate read-only connection to verify all required migrations, the named entitlement-link constraint and no reservation in flight. They report only pass/wait states and never print credentials, IDs, connection strings, legal names or the ABN. A green preflight is evidence for an owner launch decision; it does not turn payments on.
...
1. Run the target-environment launch audit with `npm run payments:check -- --preflight --strict --verify-stripe --verify-database` while Checkout is off and after any payment-setting change. The key needs Prices Read, Products Read, Checkout Sessions create/retrieve and PaymentIntents Read so the audit can verify the configured Price/Product and zero open Sessions, while the runtime database connection must resolve to `hoju_app_runtime`. Replace the full live key with a least-privilege restricted key when those permissions are confirmed. Only after every preflight item passes may the owner separately approve `PAYMENTS_ENABLED=true`.
