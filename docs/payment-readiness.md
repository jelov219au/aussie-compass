...
- Publish the Hoju Compass product provider's legal/registered identity, ABN, support contact, delivery method, refund process, privacy notice and terms before accepting money. Separately observe the transaction seller, document issuer and transaction-support details on the actual Managed Payments Checkout and every issued customer document. Treat a role as verified only when it and a populated business value are clearly customer-visible in that artifact; otherwise keep it `UNVERIFIED` and route the customer to Hoju Compass product support without guessing.
...
- Run `scripts/run-vercel-production-payment-preflight.ps1` from a fresh PowerShell process while `PAYMENTS_ENABLED=false`, passing the separately approved Neon endpoint ID and full owner-approved Production commit SHA. The outer wrapper refuses dotenv files and stale process variables, fixes Vercel CLI to the reviewed version, loads only the Vercel Production environment, and requests the Automation Bypass in the Vercel child through a masked prompt so the secret never reaches `npx` or the Vercel CLI parent. It then invokes the existing masked inner preflight, which requests the one-off `PAYMENTS_AUDIT_DB_URL`, Account-Read-only Stripe audit key and dedicated Balance-Transactions-Read accounting key, proves three distinct live restricted-key roles, and runs `npm run payments:check -- --preflight --strict --verify-stripe --verify-database`. Only after that strict payment/database audit passes does it assign the same masked accounting key and run `npm run accounting:preflight`. This verifies the configured Product, zero open Checkout Sessions, the Stripe Account and support profile, the approved Neon endpoint, the `hoju_app_runtime` and `hoju_payment_auditor` role boundaries, required migrations, no reservation in flight and accounting read access. Use only a single uninterrupted result containing outer `VERCEL_PRODUCTION_PREFLIGHT=PASS` and inner `FIRST_SALE_PREFLIGHT=PASS mode=live payments_off=yes keys=three-distinct-rk-live required_reads=verified checkout_create=not-exercised database=strict-pass secrets_printed=no`. The standalone `run-accounting-preflight.ps1` is not a second launch prerequisite in the same approval window and cannot replace this integrated result. Any missing or failed line remains `NO-GO`. The command never enables payments or exercises Checkout creation.
...
1. Run the target-environment launch audit with `.\scripts\run-vercel-production-payment-preflight.ps1 -ExpectedNeonEndpointId ep-curly-wave-a78bktnq -ExpectedProductionSha <full-owner-approved-sha>` while Checkout is off and after any payment-setting or deployment change. Reconfirm that this non-secret ID is still the separately approved Primary compute before each run. The runtime restricted key requires Prices Read, Products Read, Checkout Sessions Write and Payment Intents Read; the separate audit and accounting keys are entered only at masked prompts. Only after the outer and inner canonical PASS lines both appear may the owner separately approve `PAYMENTS_ENABLED=true`.
...
4. Confirm the ABN/GST status through the Australian Business Register and with a registered tax agent. In live Managed Payments Checkout and each issued customer document, record an exact customer-visible transaction seller, document issuer or tax-liability party only when that role and its populated business value are clearly displayed in the same artifact. Otherwise record `UNVERIFIED`, do not infer the role from `BUSINESS_*` or another artifact, and ask the registered tax agent how the gross sale, GST shown by Stripe, fees and payout belong in the sole trader's records.
