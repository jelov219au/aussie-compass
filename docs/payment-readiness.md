...
- With payments off, verify `20260823_first_sale_gate_charge_link_v2` and then apply the additive migrations in order: `docs/migrations/20260823_payment_operator_alert_outbox_v1.sql`, followed by `docs/migrations/20260823_checkout_activation_nonce_v1.sql`. Do not combine, reorder or roll them back by deleting evidence. Follow `docs/database-recovery.md` for backup and isolated restore drills; a production restore always requires explicit approval.
