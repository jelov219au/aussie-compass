...
- Run `npm run payments:check -- --preflight --strict --verify-stripe --verify-database` against the target environment while `PAYMENTS_ENABLED=false`. Inject `PAYMENTS_AUDIT_DB_URL` and the separate Account-Read-only `PAYMENTS_STRIPE_AUDIT_KEY` only for this operator command; do not store either credential in the app runtime, Preview or Production environment. The remote checks use the actual runtime key for the configured Price/Product and zero-open-Checkout proof, use the separate Stripe audit key for Account status and support-profile proof, prove the real app connection resolves to `hoju_app_runtime`, and prove the separate database audit connection resolves to `hoju_payment_auditor` with no elevated membership, unsafe role attribute, protected-table mutation, public-schema creation or payment-function execution. It then verifies all required migrations, the named entitlement-link constraint and no reservation in flight. The command reports only pass/wait states and never prints credentials, IDs, connection strings, legal names or the ABN. A green preflight is evidence for an owner launch decision; it does not turn payments on.
