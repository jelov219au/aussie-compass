...
- Run `scripts/run-production-payment-preflight.ps1` from an intentionally loaded Production operator environment while `PAYMENTS_ENABLED=false`, passing both the separately approved non-secret Neon endpoint ID and the full owner-approved Production commit SHA. Because Vercel Standard Protection covers the exact deployment origin, load the project-specific Automation Bypass value only into the wrapper process as `VERCEL_AUTOMATION_BYPASS_SECRET`; never put it on the command line, in source, in a tracked environment file or in a deployed application variable. The wrapper sends that header only to the exact `*.vercel.app` origin and removes it immediately after deployment identity succeeds. It then proves that exact SHA is the successful public Production deployment, before requesting any masked payment or database credential or starting Stripe/Neon reads. It prompts with masked input for the one-off `PAYMENTS_AUDIT_DB_URL`, Account-Read-only `PAYMENTS_STRIPE_AUDIT_KEY`, and dedicated Balance-Transactions-Read accounting key. Before any remote payment audit it requires three distinct `rk_live_` values for runtime, Account audit and accounting and refuses preloaded audit/accounting variables. It process-scopes the audit key and database URL for `npm run payments:check -- --preflight --strict --verify-stripe --verify-database`; only after that strict payment/database audit passes, it process-scopes the same masked accounting key and runs `npm run accounting:preflight`. The remote checks use the actual runtime key for the configured Price/Product and zero-open-Checkout proof, use the separate Stripe audit key for Account status and support-profile proof, prove the real app connection resolves to `hoju_app_runtime`, and prove the separate database audit connection resolves to `hoju_payment_auditor` with no elevated membership, unsafe role attribute, protected-table mutation, public-schema creation or payment-function execution. The audit refuses remote Stripe or database reads until its corresponding local mode, key, product and endpoint boundaries pass. It then verifies all required migrations, the named entitlement-link constraint, no reservation in flight, and Balance Transactions read permission on the exact prompted accounting key. The wrapper removes all temporary process variables and zeroes all three prompted unmanaged plaintext buffers before emitting its final result. Record the first-sale gate only from the exact redacted `FIRST_SALE_PREFLIGHT=PASS mode=live payments_off=yes keys=three-distinct-rk-live required_reads=verified checkout_create=not-exercised database=strict-pass secrets_printed=no` line; `STRIPE_KEY_ROLES=PASS` alone is insufficient and any FAIL, missing final PASS or interrupted run remains `NO-GO`. The standalone `run-accounting-preflight.ps1` is only for later independent revalidation after the accounting key or its permissions change; it is not a second first-sale launch prerequisite and cannot replace the integrated gate. Do not run the underlying commands with credentials assigned in shell history. The command reports only pass/wait states and never prints credentials, IDs, connection strings, legal names or the ABN. A green preflight is evidence for an owner launch decision; it does not turn payments on or exercise Checkout creation.
