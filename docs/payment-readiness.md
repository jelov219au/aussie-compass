...
- Before opening Checkout, the fail-closed preflight also reads the current Stripe Account and requires charges and payouts enabled, identity details submitted, no current or past-due requirements, and a complete customer-facing business profile. The Stripe business-profile support email must exactly match `NEXT_PUBLIC_SUPPORT_EMAIL` case-insensitively; a support address published only on the site is insufficient.
...
- Run `npm run payments:check -- --preflight --strict --verify-stripe --verify-database` inside the target deployment environment while `PAYMENTS_ENABLED=false`. Inject `PAYMENTS_AUDIT_DB_URL` only for this command using the `hoju_payment_auditor` connection; do not store that credential in the app runtime. The remote checks retrieve the configured Price with its Product, require zero existing open Checkout Sessions, prove the real app connection resolves to `hoju_app_runtime`, and prove the separate audit connection resolves to `hoju_payment_auditor` with no elevated membership, unsafe role attribute, protected-table mutation, public-schema creation or payment-function execution. It then verifies all required migrations, the named entitlement-link constraint and no reservation in flight. The command reports only pass/wait states and never prints credentials, IDs, connection strings, legal names or the ABN. A green preflight is evidence for an owner launch decision; it does not turn payments on.
...
1. Run the target-environment launch audit with `npm run payments:check -- --preflight --strict --verify-stripe --verify-database` while Checkout is off and after any payment-setting change. Set `PAYMENTS_EXPECTED_NEON_ENDPOINT_ID` to the approved environment's non-secret `ep-...` compute endpoint ID; the audit extracts endpoint IDs from both database connections, accepts direct or `-pooler` hosts only under `neon.tech`, and fails if either connection targets a different endpoint. This guards against a Preview branch with the same `neondb` database name. The key needs Prices Read, Products Read, Checkout Sessions create/retrieve and PaymentIntents Read so the audit can verify the configured Price/Product and zero open Sessions. The runtime database connection must resolve to `hoju_app_runtime`; the separately injected audit connection must resolve to `hoju_payment_auditor` and pass every effective-denial check. Replace the full live key with a least-privilege restricted key when those permissions are confirmed. Only after every preflight item passes may the owner separately approve `PAYMENTS_ENABLED=true`.
