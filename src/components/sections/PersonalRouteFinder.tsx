  ...
  return <section id="route-finder" className="scroll-mt-20 border-b border-border bg-white py-5 sm:py-6" aria-labelledby="route-finder-summary"><Container>
    ...
    <div className="grid gap-7 lg:grid-cols-[1fr_1fr] lg:gap-10">
      <div>
        ...
        <h2 id="route-finder-heading" className="mt-2 text-3xl font-semibold tracking-tight text-navy sm:text-3xl">지금 나에게 맞는<br/>세 가지부터.</h2>
      ...
      <div className="min-w-0 [overflow-wrap:anywhere] self-start rounded-2xl bg-navy p-6 text-white sm:p-8" aria-live="polite">
        ...
        <div className="mt-3 flex flex-wrap gap-x-6 gap-y-2"><button type="button" onClick={shareRecommendations} className="inline-flex min-h-11 items-center text-sm font-semibold text-white/75 hover:text-white">추천 경로 공유 ↗</button>{showPlan && <button type="button" onClick={downloadReminder} className="inline-flex min-h-11 items-center text-sm font-semibold text-white/75 hover:text-white">7일 뒤 점검 알림 +</button>}</div>
        ...
        {actionMessage && <p className="mt-3 text-xs leading-5 text-white/75" role="status" aria-live="polite">{actionMessage}</p>}
