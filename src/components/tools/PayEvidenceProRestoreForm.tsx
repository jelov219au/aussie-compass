...
const restoreNonceStorageKey = "hoju_compass_pay_evidence_pro_restore_nonce_v1";
...
const notices: Record<Exclude<RestoreNotice, "idle">, string> = {
  ...
  refunded: "환불 완료로 Pay Evidence Pack Pro 이용이 종료됐습니다.",
...
export function PayEvidenceProRestoreForm({ initialStatus }: { initialStatus?: string }) {
  ...
  async function restore(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/pay-evidence-pro/restore", {
        method: "POST",
        body: formData,
        headers: { Accept: "application/json" },
      });
      const body = await response.json().catch(() => null) as { code?: string; destination?: string } | null;
      if (response.ok && body?.code === "restore_ready" && body.destination === "/pay-evidence-pro/workspace") {
  ...
  return (
    <>
      ...
      <form action="/api/pay-evidence-pro/restore" method="post" onSubmit={restore} className="mt-8 border border-navy/15 bg-white p-5 sm:p-6">
