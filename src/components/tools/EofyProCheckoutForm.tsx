...
import type { EofyProEntry } from "@/lib/eofyProAttribution";

export function EofyProCheckoutForm({
  testMode,
  entry,
}: {
  testMode: boolean;
  ...
  return (
    <form
      action="/api/checkout/eofy-pro"
      method="post"
      onSubmit={() => track("Checkout Started", { product: "eofy_pro", entry })}
    ...
    >
      ...
      >
        {testMode ? "A$9.90 테스트 결제 시작" : "A$9.90에 EOFY 준비 정리하기"}
      </button>
    </form>
  );
}
