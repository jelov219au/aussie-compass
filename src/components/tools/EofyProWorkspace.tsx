...
function updateEmptySections(current: EofyDraft, section: "expenses" | "documents", confirmed: boolean): EofyDraft["emptySections"] {
  const next = { ...current.emptySections };
  if (confirmed) next[section] = true;
  else delete next[section];
  return Object.keys(next).length ? next : undefined;
}

export function EofyProWorkspace() {
  const [completedTaxYearStart] = useState(() => getMostRecentCompletedEofyStartYear());
  const [draft, setDraft] = useState<EofyDraft>({ taxYear: formatEofyTaxYear(completedTaxYearStart), incomeStatuses: {}, expenses: [], questions: [] });
  const [knownTaxYears, setKnownTaxYears] = useState<string[]>([]);
  const taxYearOptions = [...new Set([
    ...[completedTaxYearStart - 1, completedTaxYearStart, completedTaxYearStart + 1].map(formatEofyTaxYear),
    ...knownTaxYears,
    draft.taxYear,
  ])];
  ...
  const emptySectionsToReview = Number(!draft.expenses.length && draft.emptySections?.expenses !== true && !handoffReviewed)
    + Number(!documents.length && draft.emptySections?.documents !== true && !handoffReviewed);
  const progress = calculateEofyProgress({
    incomeReady,
    incomeTotal: incomeSources.length,
    expenseCount: draft.expenses.length,
    expenseReady: draft.expenses.length - expenseReview.length,
    documentCount: documents.length,
    documentReady: documents.length - documentsToReview.length,
    expensesEmptyConfirmed: draft.emptySections?.expenses === true,
    documentsEmptyConfirmed: draft.emptySections?.documents === true,
    handoffReviewed,
  });

  const setEmptySectionConfirmation = (section: "expenses" | "documents", confirmed: boolean) => setDraft((current) => {
    return { ...current, emptySections: updateEmptySections(current, section, confirmed) };
  });
  ...
  const addDocument = () => {
    ...
    setDraft((current) => (current.documents ?? []).length >= eofyDocumentLimit ? current : {
      ...current,
      documents: [...(current.documents ?? []), item],
      emptySections: updateEmptySections(current, "documents", false),
    });
  };
  const removeDocument = (id: string) => setDraft((current) => {
    const documents = (current.documents ?? []).filter((document) => document.id !== id);
    return {
      ...current,
      documents,
      ...(documents.length ? {} : { emptySections: updateEmptySections(current, "documents", false) }),
    };
  });
  ...
  const addExpense = () => {
    ...
    setDraft((current) => current.expenses.length >= eofyExpenseLimit ? current : {
      ...current,
      expenses: [...current.expenses, expense],
      emptySections: updateEmptySections(current, "expenses", false),
    });
  };
  const removeExpense = (id: string) => setDraft((current) => {
    const expenses = current.expenses.filter((expense) => expense.id !== id);
    return {
      ...current,
      expenses,
      ...(expenses.length ? {} : { emptySections: updateEmptySections(current, "expenses", false) }),
    };
  });
  ...
  return <div className="grid items-start gap-8 xl:grid-cols-[minmax(0,0.9fr)_minmax(34rem,1.1fr)]">
    ...
    <div className="space-y-8">
      ...
      <section className="border border-border bg-white p-5 sm:p-7" aria-labelledby="eofy-documents-heading">
        ...
        <p className="mt-2 text-xs leading-5 text-muted">기존 v1 백업도 복원할 수 있습니다. 문서 기록을 담은 v2와 0건 확인을 담은 v3 백업은 최신 사이트에서 복원하세요.</p>
        <div className="mt-5 space-y-4">{documents.map((item, index) => <fieldset key={item.id} className="min-w-0 border border-border p-4" aria-describedby={`eofy-document-help eofy-document-issues-${index}`}>
          ...
          <button type="button" aria-label={`문서 ${index + 1} 삭제`} className="mt-2 min-h-11 min-w-11 text-sm text-muted hover:text-red-700" onClick={() => removeDocument(item.id)}>삭제</button>
        </fieldset>)}</div>
    ...
    <div className="space-y-8 xl:sticky xl:top-24">
      <section className="border border-border bg-white p-5 shadow-sm sm:p-7" aria-labelledby="expense-register-heading"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-gold-ink">Evidence register</p><h2 id="expense-register-heading" className="mt-2 text-xl font-semibold text-navy">공제 후보 기록</h2></div><div className="text-right"><p className="font-mono text-2xl text-navy">A${candidateTotal.toFixed(2)}</p><p className="text-xs text-muted">유효한 금액 합계 · 공제액 아님</p>{amountTotals.excluded ? <p className="mt-1 text-xs text-red-700">합계 미포함 {amountTotals.excluded}개 · 미입력 또는 확인 필요</p> : null}</div></div>
        ...
        {!draft.expenses.length ? <label className="mt-5 flex min-h-11 items-center gap-3 border border-border bg-surface px-4 py-3 text-sm text-navy"><input type="checkbox" checked={draft.emptySections?.expenses === true} disabled={!loaded} onChange={(event) => setEmptySectionConfirmation("expenses", event.target.checked)} className="h-5 w-5 accent-[var(--color-gold)]" />이 회계연도에는 기록할 지출 후보가 없음을 확인했습니다.</label> : null}
      </section>

      <section className="border border-border bg-surface p-5 sm:p-7" aria-labelledby="eofy-handoff-review-heading"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-gold-ink">Accountant handoff review</p><h2 id="eofy-handoff-review-heading" className="mt-2 text-xl font-semibold text-navy">전달 전에 따로 볼 항목</h2></div><p className="font-mono text-2xl text-navy">{handoffReview.totalFlags + documentsToReview.length + emptySectionsToReview}</p></div><p className="mt-3 text-sm leading-6 text-muted">같은 항목이 여러 범주에 표시될 수 있습니다. 이 분류는 공제 가능 여부를 판단하지 않고, 상담 전에 확인할 기록을 놓치지 않도록 돕습니다.</p>
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          ...
          ].map((group) => <article key={group.title} className="border border-border bg-white p-4"><div className="flex items-start justify-between gap-3"><div><h3 className="text-sm font-semibold text-navy">{group.title}</h3><p className="mt-1 text-xs leading-5 text-muted">{group.detail}</p></div><span className="font-mono text-lg text-gold-ink">{group.ids.length}</span></div>{group.ids.length ? <ul className="mt-3 space-y-1 border-t border-border pt-3">{group.ids.map((id) => <li key={id} className="text-xs leading-5 text-navy">{expenseName(id)}</li>)}</ul> : <p className="mt-3 border-t border-border pt-3 text-xs text-muted">표시된 항목 없음</p>}</article>)}
        </div>
        <p className="mt-4 text-xs leading-5 text-muted">소득 자료 미완료: {handoffReview.incomeNotReady.length}개 · 표시된 지출 후보: {handoffReview.flaggedExpenseCount}개 · 별도 확인할 문서: {documentsToReview.length}개 · 0건 확인 전 범주: {emptySectionsToReview}개</p>
