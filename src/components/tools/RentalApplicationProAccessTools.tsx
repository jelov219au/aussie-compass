...
export function RentalApplicationProAccessTools() {
  ...
  async function releaseAccess(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/rental-application-pro/access/release", {
        method: "POST",
      ...
      if (!response.ok || result?.released !== true || result.destination !== "/rental-application-pro?access=released") {
  ...
  return (
    <section className="mt-10 border border-navy/15 bg-white p-5 sm:p-6" aria-labelledby="rental-pro-access-heading">
      <p className="text-xs font-semibold uppercase tracking-[0.18em] text-gold">구매 내역 다시 찾기</p>
      <h2 id="rental-pro-access-heading" className="mt-2 text-xl font-semibold text-navy">다른 기기에서 이용권 복구하기</h2>
      ...
      <div className="mt-6 border-t border-border pt-5">
        ...
        <p className="mt-2 text-sm leading-6 text-muted">이 기기의 Rental Application Pack Pro 접근 쿠키만 제거합니다. 구매 이용권과 다른 기기의 접근은 유지됩니다.</p>
        <form action="/api/rental-application-pro/access/release" method="post" onSubmit={releaseAccess} className="mt-3 w-full sm:w-auto">
        ...
        {releaseUnknown && (
          <div className="mt-4 border-l-2 border-gold bg-gold/10 p-4 text-sm leading-6 text-navy" role="alert" aria-live="assertive">
            <p>요청 결과를 확인하지 못했습니다. 다시 결제하지 말고 이 기기의 Rental Pack Pro 접근 상태를 다시 확인하거나 고객지원에서 구매 내역을 확인해 주세요.</p>
            <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:flex-wrap">
              <Link href="/rental-application-pro/workspace" className="inline-flex min-h-11 w-full items-center justify-center border border-navy px-4 py-2 font-semibold sm:w-auto">접근 상태 다시 확인</Link>
