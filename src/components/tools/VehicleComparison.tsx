...
export function VehicleComparison() {
  ...
  <div className="mt-5 space-y-3">{checks.map(([field,label,detail])=><label key={field} className="flex cursor-pointer gap-3"><input type="checkbox" checked={Boolean(vehicle[field])} onChange={(e)=>update(vehicle.id,field,e.target.checked)} className="mt-0.5 h-5 w-5 accent-[var(--color-gold)]" /><span><span className="block text-sm font-medium text-navy">{label}</span><span className="block text-xs leading-5 text-muted">{detail}</span></span></label>)}</div></article>})}</div>
  <button type="button" onClick={()=>setVehicles([makeVehicle(0),makeVehicle(1)])} className="mt-6 min-h-11 rounded-lg border border-border px-4 text-sm font-semibold text-navy">비교표 초기화</button></section>;
}
