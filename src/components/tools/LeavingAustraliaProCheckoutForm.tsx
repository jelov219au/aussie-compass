...
import type { LeavingAustraliaProEntry } from "@/lib/leavingAustraliaProAttribution";

export function LeavingAustraliaProCheckoutForm({
  testMode,
  entry,
}: {
  testMode: boolean;
  entry: LeavingAustraliaProEntry;
}) {
  const [accepted, setAccepted] = useState(false);

  return (
    <form
      action="/api/checkout/leaving-australia-pro"
      method="post"
      onSubmit={() => track("Checkout Started", { product: "leaving_australia_pro", entry })}
    ...
    >
      ...
      <label className="flex cursor-pointer items-start gap-3 text-sm leading-6 text-navy">
        ...
        <span>
          {testMode
            ...
            : "A$12.90 1회 결제와 디지털 제공·이용·환불 조건을 확인했습니다."}
        </span>
      </label>
      ...
      >
        {testMode ? "A$12.90 테스트 결제 시작" : "A$12.90에 출국 정리하기"}
      </button>
    </form>
  );
}
