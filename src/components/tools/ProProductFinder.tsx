...
const options: { id: Situation; label: string; detail: string }[] = [
  ...
  { id: "car", label: "중고차 구매를 검토 중", detail: "검사 질문·수리 약속·증빙·재확인" },
];

const recommendations = {
  job: { href: "/resume-pro", name: "Resume Pro", price: "A$9.90", description: "지원하고 싶은 공고가 생겼을 때, 내 실제 경력을 회사별 이력서와 커버레터로 정리해요.", cta: "이 공고에 맞춰 지원 준비하기", freeHref: "/resume-job-ad-checker", freeLabel: "결제 전에 내 공고로 무료 점검하기" },
  ...
  car: { href: "/car-purchase-pro", name: "Car Purchase Pack Pro", price: "가격 미정", description: "검사 뒤 질문, 판매자의 수리 약속과 증빙, 독립 재확인, 최종 결정을 한 거래노트로 이어가는 방식을 준비하고 있어요.", cta: "중고차 거래노트 준비 방식 보기", freeHref: "/used-car-comparison", freeLabel: "무료 후보·비용 비교표 사용하기" },
} as const;
...
export function ProProductFinder({ resumeProLive, rentalProLive, packAvailability }: ProProductFinderProps) {
  ...
  const selectedProductLive = situation === "job" ? resumeProLive : situation === "home" ? rentalProLive : situation === "car" ? false : packAvailability[situation];
