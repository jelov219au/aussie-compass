...
export function ProProductFinder({ resumeProLive, rentalProLive }: { resumeProLive: boolean; rentalProLive: boolean }) {
  const [situation, setSituation] = useState<Situation>("job");
  const product = recommendations[situation];
  const selectedProductLive = situation === "job" ? resumeProLive : situation === "home" && rentalProLive;
