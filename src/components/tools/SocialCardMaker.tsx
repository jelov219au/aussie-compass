...
const presets = [
  ...
  { label: "은행 영어", eyebrow: "은행에서 수수료를 확인할 때", title: "Are there any monthly or ATM fees?", body: "월 관리비나 ATM 수수료가 있나요? 계좌를 열기 전에 비용과 조건을 글로 확인해 두세요.", cta: "은행에서 쓰는 문장 더 보기", path: "/english-phrase-cards" },
  { label: "렌트 영어", eyebrow: "집을 보고 돈을 보내기 전에", title: "Could you send me the agreement before I pay?", body: "돈을 보내기 전에 계약서를 보내주실 수 있나요? Bond와 포함 비용도 글로 확인하세요.", cta: "렌트할 때 쓰는 문장 더 보기", path: "/english-phrase-cards" },
  { label: "직장 영어", eyebrow: "첫 근무 전에 시급을 확인할 때", title: "Could you confirm my hourly rate in writing?", body: "시급과 Classification을 글로 확인해 주실 수 있나요? 첫 근무 전에 기록을 남겨두세요.", cta: "직장에서 쓰는 문장 더 보기", path: "/english-phrase-cards" },
  { label: "병원 영어", eyebrow: "병원에서 통역이 필요할 때", title: "I need a Korean interpreter, please.", body: "한국어 통역이 필요합니다. 증상, 동의서와 약 복용처럼 중요한 내용은 정확히 이해할 때까지 물어보세요.", cta: "병원에서 쓰는 문장 더 보기", path: "/english-phrase-cards" },
...
export function SocialCardMaker({ baseUrl, initialContent }: { baseUrl: string; initialContent?: SocialCardInitialContent }) {
  ...
  const [activePresetLabel, setActivePresetLabel] = useState(initialContent ? "직접 편집" : presets[0].label);
  ...
  async function downloadCard() {
    ...
    canvas.toBlob((blob) => { if (!blob) return; const url = URL.createObjectURL(blob); const anchor = document.createElement("a"); const fileLabel = activePresetLabel.toLocaleLowerCase("ko-KR").replace(/\s+/g, "-"); anchor.href = url; anchor.download = `hoju-compass-${fileLabel}-${format}.png`; document.body.appendChild(anchor); anchor.click(); anchor.remove(); window.setTimeout(() => URL.revokeObjectURL(url), 0); setStatus("PNG 이미지를 저장했습니다."); }, "image/png");
  }
