...
function createApplication(id: string, propertyLabel = ""): RentalApplication {
  return { id, propertyLabel, suburb: "", jurisdiction: "", weeklyRent: "", agentName: "", moveDate: "", leaseTerm: "12 months", stage: "shortlist", applicationDate: "", nextActionDate: "", notes: "", statuses: {}, privacyChecks: {}, messages: { application: "", inspection: "", followUp: "" } };
}
...
export function RentalApplicationWorkspace() {
  ...
  const downloadSummary = () => {
    const lines = ["HOJU COMPASS — RENTAL APPLICATION PACK", `Property: ${active.propertyLabel || "Not set"}`, `Suburb: ${active.suburb || "Not set"}`, `State or territory: ${activeJurisdiction ? `${activeJurisdiction.name} (${activeJurisdiction.code})` : "Not set"}`, `Weekly rent: ${active.weeklyRent ? `A$${active.weeklyRent}` : "Not set"}`, `Stage: ${stageLabel(active.stage)}`, `Move-in: ${active.moveDate || "Not set"}`, `Next action: ${active.nextActionDate || "Not set"}`, "", "STATE OR TERRITORY CHECK", ...(activeJurisdiction ? [...activeJurisdiction.checkpoints.map((item) => `- ${item}`), `- Official source: ${activeJurisdiction.href}`] : ["- Select a state or territory and check the current official guidance before applying."]), "", "DOCUMENT STATUS", ...documents.map((item) => `- [${statusLabels[active.statuses[item.id] ?? "todo"]}] ${item.title}`), "", "PRIVACY CHECK", ...privacyChecks.map(([id, title]) => `- [${active.privacyChecks[id] ? "Checked" : "Not checked"}] ${title}`), "", "ITEMS TO REVIEW", ...(reviewItems.length ? reviewItems.map((item) => `- ${item.title}: ${item.caution ?? item.detail}`) : ["- None marked"]), "", "APPLICATION NOTE", active.messages.application || "Not created", "", "Preparation summary only. Check current official rules. Do not include TFN, bank login details or identity document numbers."];
  ...
  return <div className="space-y-8">
    ...
    <section id="rental-application-print" className="hidden" aria-label="렌트 신청 준비 패키지 인쇄본"><header><p>HOJU COMPASS / RENTAL APPLICATION PACK</p><h1>{active.propertyLabel || "Rental application"}</h1><p>{active.suburb || "Suburb not set"}{activeJurisdiction ? `, ${activeJurisdiction.code}` : ""} · {stageLabel(active.stage)} · {progress}% documents ready</p></header><section><h2>Application brief</h2><dl><div><dt>State or territory</dt><dd>{activeJurisdiction ? `${activeJurisdiction.name} (${activeJurisdiction.code})` : "Not set"}</dd></div><div><dt>Weekly rent</dt><dd>{active.weeklyRent ? `A$${active.weeklyRent}` : "Not set"}</dd></div><div><dt>Preferred move-in</dt><dd>{formatDate(active.moveDate)}</dd></div><div><dt>Lease term</dt><dd>{active.leaseTerm}</dd></div><div><dt>Application date</dt><dd>{formatDate(active.applicationDate)}</dd></div><div><dt>Next action</dt><dd>{formatDate(active.nextActionDate)}</dd></div></dl></section>{activeJurisdiction ? <section><h2>{activeJurisdiction.code} official checkpoints</h2><ul>{activeJurisdiction.checkpoints.map((item) => <li key={item}><span>{item}</span><strong>Check</strong></li>)}</ul><p>{activeJurisdiction.authority}: {activeJurisdiction.href}</p></section> : null}<section><h2>Document readiness</h2><ul>{documents.map((item) => <li key={item.id}><span>{item.title}</span><strong>{statusLabels[active.statuses[item.id] ?? "todo"]}</strong></li>)}</ul></section><section><h2>Privacy check</h2><ul>{privacyChecks.map(([id, title]) => <li key={id}><span>{title}</span><strong>{active.privacyChecks[id] ? "Checked" : "Not checked"}</strong></li>)}</ul></section><section><h2>Application note</h2><p className="whitespace-pre-wrap">{active.messages.application || "Not created"}</p></section>{active.notes ? <section><h2>Preparation notes</h2><p className="whitespace-pre-wrap">{active.notes}</p></section> : null}<footer>Preparation summary only, not a rental application or legal advice. Check current official rules and keep sensitive information outside this file.</footer></section>
  </div>;
}
