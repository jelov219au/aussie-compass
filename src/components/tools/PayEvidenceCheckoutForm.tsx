...
export function PayEvidenceCheckoutForm({ testMode }: { testMode: boolean }) {
  const [accepted, setAccepted] = useState(false);
  return (
    <form action="/api/checkout/pay-evidence-pro" method="post" onSubmit={() => track("Checkout Started", { product: "pay_evidence_pro", mode: testMode ? "test" : "live" })} className="w-full max-w-xl border border-navy/15 bg-white p-4 sm:p-5">
      <label className="flex cursor-pointer items-start gap-3 text-sm leading-6 text-navy">
        <input type="checkbox" name="terms_accepted" value="yes" required checked={accepted} onChange={(event) => setAccepted(event.target.checked)} className="mt-1 h-5 w-5 shrink-0 accent-[#c6a34b]" />
