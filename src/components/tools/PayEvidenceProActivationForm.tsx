...
const activationStorageKey = "hoju_compass_pay_evidence_pro_activation_v1";
...
export function PayEvidenceProActivationForm({
  initialSessionId,
  initialNotice,
}: {
  ...
  async function activate(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/pay-evidence-pro/access/activate", {
        method: "POST",
        body: formData,
        headers: { Accept: "application/json" },
      });
      const body = await response.json().catch(() => null) as { code?: string; destination?: string } | null;
      if (response.ok && body?.code === "activation_ready" && body.destination === "/pay-evidence-pro/workspace") {
        clearStoredActivation();
      ...
      if (body?.destination?.startsWith("/pay-evidence-pro/")) {
        window.location.assign(body.destination);
        return;
      }
  ...
  return (
    <div>
      <form action="/api/pay-evidence-pro/access/activate" method="post" onSubmit={activate}>
        <button type="submit" disabled={!activation || submitting} aria-busy={submitting} className="inline-flex min-h-12 items-center justify-center bg-gold px-5 py-3 text-sm font-semibold text-navy disabled:cursor-wait disabled:opacity-50">
          {submitting ? "이용권 확인 중…" : initialNotice === "ready" ? "Pay Evidence Pack Pro 열기" : "재결제하지 않고 이용권 다시 확인"}
        </button>
      </form>
