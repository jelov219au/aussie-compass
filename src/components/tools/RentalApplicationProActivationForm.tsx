...
const activationStorageKey = "hoju_compass_rental_application_pro_activation_v1";
...
export function RentalApplicationProActivationForm({
  initialSessionId,
  initialNotice,
}: {
  ...
  async function activate(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/rental-application-pro/access/activate", {
        method: "POST",
        body: formData,
        headers: { Accept: "application/json" },
      });
      const body = await response.json().catch(() => null) as { code?: string; destination?: string } | null;
      if (response.ok && body?.code === "activation_ready" && body.destination === "/rental-application-pro/workspace") {
        clearStoredActivation();
      ...
      if (body?.destination?.startsWith("/rental-application-pro/")) {
        window.location.assign(body.destination);
        return;
      }
  ...
  return (
    <div>
      <form action="/api/rental-application-pro/access/activate" method="post" onSubmit={activate}>
        <button type="submit" disabled={!activation || submitting} aria-busy={submitting} className="inline-flex min-h-12 items-center justify-center bg-gold px-5 py-3 text-sm font-semibold text-navy disabled:cursor-wait disabled:opacity-50">
          {submitting ? "이용권 확인 중…" : initialNotice === "ready" ? "Rental Pack Pro 열기" : "재결제하지 않고 이용권 다시 확인"}
        </button>
      </form>
