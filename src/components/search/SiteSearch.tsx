...
const suggestions = ["TFN", "Bond", "전기 요금", "세후 급여", "영문 이력서", "커버레터", "이력서 양식", "택스 리턴", "Super 환급", "중고거래", "통역", "교통"];
...
export function SiteSearch({ items }: { items: SearchItem[] }) {
  ...
  return <>
    ...
    <div className="mt-10">
      {results.length ? intent === "resume" || intent === "resume-pro-direct" ? (
        <section className="mb-12" aria-labelledby="search-resume-recommendations">
          <div className="flex items-end justify-between border-b border-navy/20 pb-4">
            <h2 id="search-resume-recommendations" className="text-2xl font-semibold text-navy">이력서 준비 추천 순서</h2>
        ...
        <div className="border-y border-border py-16 text-center">
          ...
          <p className="mt-2 text-sm text-muted">단어를 조금 짧게 입력하거나 위 추천 검색어를 골라보세요.</p>
