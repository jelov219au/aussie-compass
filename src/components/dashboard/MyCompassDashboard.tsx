...
export function MyCompassDashboard({ resourceArticles }: { resourceArticles: ResourceSummary[] }) {
  ...
  return <div className="mt-10"><section className="grid gap-6 border-y border-navy/20 py-7 sm:grid-cols-[1fr_auto] sm:items-end"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-gold">이 기기에 남아 있는 기록</p><h2 className="mt-2 text-2xl font-semibold text-navy">이어볼 수 있는 기록이 {active.length + bookmarks.length + readArticles.length}개 있어요.</h2><p className="mt-2 text-sm leading-6 text-muted">이름이나 연락처, 급여와 예산 금액은 이 요약 화면에 표시하지 않아요. 현재 브라우저에 저장된 진행 상태만 불러옵니다.</p></div><div className="flex flex-wrap items-center gap-5"><Link href="/data-transfer" className="inline-flex min-h-11 items-center border-b-2 border-navy text-sm font-semibold text-navy hover:border-gold">백업·기기 이전</Link><button type="button" onClick={refresh} className="min-h-11 border-b-2 border-gold text-sm font-semibold text-navy">진행 상황 새로고침</button></div></section>
    ...
    {readArticles.length > 0 && <section className="mt-12 border-t border-navy/20 pt-7" aria-labelledby="read-articles"><div className="flex items-end justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-gold">전에 읽어본 글</p><h2 id="read-articles" className="mt-1 text-2xl font-semibold text-navy">읽은 실용 자료</h2></div><span className="font-mono text-xs text-muted">{String(readArticles.length).padStart(2,"0")}</span></div><ul className="mt-4 grid gap-x-8 md:grid-cols-2">{readArticles.map((item)=><li key={item.href} className="border-b border-border"><Link href={item.href} className="group flex min-h-20 items-center justify-between gap-4 py-4"><span><strong className="block text-navy">{item.title}</strong><span className="mt-1 block text-xs text-muted">{new Date(item.completedAt).toLocaleDateString("ko-KR")} 읽음</span></span><span className="text-xl text-navy transition group-hover:translate-x-1">→</span></Link></li>)}</ul></section>}
