...
export function MyCompassDashboard() {
  ...
  return <div className="mt-10"><section className="grid gap-6 border-y border-navy/20 py-7 sm:grid-cols-[1fr_auto] sm:items-end"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-gold">Saved on this device</p><h2 className="mt-2 text-2xl font-semibold text-navy">현재 기기에 {active.length + bookmarks.length}개의 기록이 있습니다.</h2><p className="mt-2 text-sm leading-6 text-muted">이 화면은 브라우저에 이미 저장된 진행 상태만 읽으며, 이름·연락처·급여·예산 금액은 요약 화면에 표시하지 않습니다.</p></div><button type="button" onClick={refresh} className="min-h-11 border-b-2 border-gold text-sm font-semibold text-navy">진행 상황 새로고침</button></section>
