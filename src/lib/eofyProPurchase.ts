...
import { eofyProProduct } from "@/lib/commerce";
import { getStripe, getStripeSecretMode } from "@/lib/stripe";

export async function getVerifiedEofyProCheckout(sessionId: string) {
  ...
  try {
    ...
    const paid = session.payment_status === "paid"
      ...
      && session.metadata?.product_code === "eofy_pro"
      && session.metadata?.billing_model === "one_time"
      && session.currency === eofyProProduct.currency
      && session.amount_total === eofyProProduct.priceCents
      && session.line_items?.data.length === 1
      && item?.quantity === 1
      && item?.currency === eofyProProduct.currency
      && item.amount_total === eofyProProduct.priceCents;
