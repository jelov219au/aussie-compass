...
import { payEvidenceProProduct } from "@/lib/commerce";
import { getStripe, getStripeSecretMode } from "@/lib/stripe";

export async function getVerifiedPayEvidenceProCheckout(sessionId: string) {
  ...
  try {
    ...
    const paid = session.payment_status === "paid"
      ...
      && session.currency === payEvidenceProProduct.currency
      && session.amount_total === payEvidenceProProduct.priceCents
      && session.line_items?.data.length === 1
      && item?.quantity === 1
      && item?.currency === payEvidenceProProduct.currency
      && item.amount_total === payEvidenceProProduct.priceCents;
