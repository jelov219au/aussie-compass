...
export const metadata = createPageMetadata({ title: "취업·렌트·급여·세금·중고차 준비 도구 비교 | Hoju Compass Pro", description: "호주 취업 지원, 렌트 신청, 급여 확인, 택스 리턴, 귀국 정리와 중고차 구매에서 반복 작업을 줄이는 Pro 도구의 준비 내용과 이용 상태를 비교하세요.", path: "/pro" });
...
export default function ProPage() {
  ...
  const packAvailability = {
    ...
    leave: products.some((product) => product.id === "leaving-australia-pro" && product.live),
  };

  return <><BreadcrumbJsonLd items={[{ name: "홈", path: "/" }, { name: "Hoju Compass Pro", path: "/pro" }]} /><Header /><main className="site-screen pro-screen">
    <section className="border-b border-navy/15 py-8 sm:py-12"><Container><Link href="/" className="inline-flex min-h-11 items-center text-sm font-medium text-muted hover:text-navy">&larr; 홈으로 돌아가기</Link><div className="mt-5 grid gap-6 lg:grid-cols-[1fr_22rem] lg:items-end"><div className="max-w-4xl"><p className="text-xs font-semibold uppercase tracking-[0.2em] text-gold-ink">Hoju Compass Pro</p><h1 className="mt-4 text-3xl font-semibold leading-[1.15] tracking-[-0.04em] text-navy [word-break:keep-all] sm:text-5xl">혼자 준비하느라 쓰는 시간을,<br /><span className="font-normal text-navy-light">조금 덜어드릴게요.</span></h1><p className="mt-6 max-w-2xl text-base leading-7 text-muted sm:text-lg">해야 할 일은 무료 안내에서 먼저 확인하세요. 문서나 정리본이 필요할 때만 내 상황에 맞는 Pro 도구를 골라 이용할 수 있어요.</p></div><aside className="border-l-2 border-gold pl-6 text-sm leading-7 text-muted"><strong className="block text-navy">{availabilityTitle}</strong>{availabilityDetail}</aside></div></Container></section>
    <section className="border-y border-navy/15 bg-white py-14 sm:py-20"><Container><div className="grid gap-7 lg:grid-cols-[1fr_20rem] lg:items-end"><div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-gold-ink">Pro 도구 한눈에 보기</p><h2 className="mt-3 text-3xl font-semibold tracking-tight text-navy sm:text-5xl">무엇을 준비하는지 한눈에.</h2></div><p className="text-sm leading-7 text-muted">{priceSummary}</p></div><ol className="mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-3">{products.map((product) => { const resumeProduct = product.name === "Resume Pro"; const liveProduct = product.live; return <li key={product.href}><article id={product.id} className={`scroll-mt-24 flex h-full min-h-[25rem] flex-col overflow-hidden rounded-3xl border-2 bg-background shadow-[0_10px_28px_rgba(26,39,68,0.06)] ${liveProduct ? "border-gold" : "border-navy/10"}`}><div className="flex items-start justify-between gap-4 bg-[#e8efee] p-6"><TopicIcon name={product.icon} /><span className={`rounded-full px-3 py-1.5 text-xs font-semibold ${liveProduct ? "bg-[#e3f3e8] text-[#24623b]" : "bg-white text-muted"}`}>{product.status}</span></div><div className="flex flex-1 flex-col p-6"><p className="text-xs font-semibold uppercase tracking-[0.14em] text-gold-ink">{product.index} · {product.label}</p><div className="mt-3 flex flex-wrap items-end justify-between gap-4"><h3 className="text-2xl font-semibold text-navy">{product.name}</h3><div className="text-right"><p className="text-xl font-semibold text-navy">{product.price}</p><p className="text-[0.68rem] text-muted">{product.priceNote}</p></div></div><dl className="mt-5"><dt className="text-xs font-semibold text-navy">Pro로 준비할 수 있어요</dt><dd className="mt-1 text-sm leading-7 text-muted">{product.outcome}</dd></dl><div className="mt-5 rounded-2xl border border-navy/10 bg-white p-4"><span className="text-[0.68rem] font-semibold uppercase tracking-[0.12em] text-muted">무료로 먼저 할 수 있는 일</span><p className="mt-1 text-sm font-semibold text-navy">{product.free}</p></div><div className="mt-auto grid gap-3 pt-7">{resumeProduct ? <ResumeProCtaLink href="/resume-pro?from=pro-catalog-card" surface={resumeFunnelSurfaces.proCatalogCard} context={resumeFunnelContexts.proCatalog} className={actionClass(liveProduct ? "primary" : "secondary", "w-full")}>{liveProduct ? `Resume Pro 보기 · ${product.price}` : "기능 자세히 보기"} <span aria-hidden="true">→</span></ResumeProCtaLink> : <Link href={product.href} className={actionClass(liveProduct ? "primary" : "secondary", "w-full")}>{liveProduct ? `${product.name} 보기 · ${product.price}` : "준비 방식 살펴보기"} <span aria-hidden="true">→</span></Link>}{resumeProduct ? <ResumeProProofLink entry="pro-catalog-card" className={actionClass("tertiary", "justify-self-center")}>내 공고로 무료 점검</ResumeProProofLink> : <Link href={product.freeHref} className={actionClass("tertiary", "justify-self-center")}>{product.free}</Link>}</div></div></article></li>; })}</ol></Container></section>
    <section className="py-14 sm:py-20"><Container><ProProductFinder resumeProLive={resumeProLive} rentalProLive={rentalProLive} packAvailability={packAvailability} /></Container></section>
