...
export default function RentalApplicationProPage() {
  return <>
    ...
    <main>
      ...
      <section className="bg-navy py-12 text-white sm:py-16"><Container className="grid gap-6 sm:grid-cols-[1fr_auto] sm:items-end"><div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-gold">Local-first product</p><h2 className="mt-3 text-3xl font-semibold tracking-tight">원본 서류는 받지 않는 도구로 시작합니다.</h2><p className="mt-3 max-w-2xl text-sm leading-7 text-white/65">제품은 브라우저 안에서 준비 상태와 소개문만 저장하는 방향입니다. 신청서 제출과 파일 업로드는 에이전트의 검증된 채널에서 직접 진행하세요.</p></div><Link href="/property-inspection-checklist" className="inline-flex min-h-12 items-center justify-center bg-gold px-5 text-sm font-semibold text-navy hover:bg-white">무료 집 점검 도구 →</Link></Container></section>
    </main>
    <Footer />
  </>;
}
