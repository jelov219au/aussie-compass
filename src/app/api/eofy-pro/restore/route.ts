...
import {
  createEofyRestoreAccessSession,
  hashEofyRestoreCode,
...
function restoreResponse(request: NextRequest, code: string, status: number, destination?: string) {
  ...
  const location = destination ?? `/eofy-pro/restore?status=${encodeURIComponent(code.replace("restore_", ""))}`;
...
export async function POST(request: NextRequest) {
  ...
  try {
    const restoreHash = hashEofyRestoreCode(code);
    const { nonceHash, accessSession } = createEofyRestoreAccessSession(restoreHash, nonce);
    ...
    if ((result.outcome === "consumed" || result.outcome === "idempotent") && result.entitlement) {
      ...
      return restoreResponse(request, "restore_ready", 200, "/eofy-pro/workspace");
    }
