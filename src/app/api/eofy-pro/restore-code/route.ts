...
import {
  createEofyRestoreCode,
  getActiveEofyProEntitlement,
...
export async function POST(request: NextRequest) {
  ...
  if (!entitlement || !store) {
    return NextResponse.json({ error: "Active EOFY Pack Pro access is required." }, {
      status: 401,
      headers: { "Cache-Control": "no-store" },
    });
  }

  const restore = createEofyRestoreCode();
