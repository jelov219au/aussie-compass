...
import {
  createLeavingAustraliaRestoreAccessSession,
  hashLeavingAustraliaRestoreCode,
...
function restoreResponse(request: NextRequest, code: string, status: number, destination?: string) {
  ...
  const location = destination ?? `/leaving-australia-pro/restore?status=${encodeURIComponent(code.replace("restore_", ""))}`;
...
export async function POST(request: NextRequest) {
  ...
  try {
    const restoreHash = hashLeavingAustraliaRestoreCode(code);
    const { nonceHash, accessSession } = createLeavingAustraliaRestoreAccessSession(restoreHash, nonce);
    ...
    if ((result.outcome === "consumed" || result.outcome === "idempotent") && result.entitlement) {
      ...
      return restoreResponse(request, "restore_ready", 200, "/leaving-australia-pro/workspace");
    }
