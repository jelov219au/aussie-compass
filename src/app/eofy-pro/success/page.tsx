...
import { EofyProActivationForm } from "@/components/tools/EofyProActivationForm";
import { Container } from "@/components/ui/Container";
import { getConfiguredEntitlementStore } from "@/lib/neonEntitlementStore";
import { isEofyEntitlementSessionConfigured } from "@/lib/eofyProAccess";
import { getVerifiedEofyProCheckout } from "@/lib/eofyProPurchase";

export const metadata: Metadata = {
  title: "EOFY Pack Pro 결제 확인 | Hoju Compass",
  description: "EOFY Pack Pro 결제와 이용권 처리 상태를 안전하게 확인합니다.",
...
const notices: Record<Exclude<PostPurchaseNotice, "ready">, string> = {
  ...
  refunded: "환불 완료로 EOFY Pack Pro 이용권이 종료됐습니다. 결제 내역은 고객지원에서 확인할 수 있습니다.",
...
export default async function EofyProSuccessPage({ searchParams }: Props) {
  ...
  if (sessionId) {
    ...
    if (session) {
      ...
      entitlementActive = Boolean(await store?.findActiveByCheckoutSession(session.id, "eofy_pro"));
    }
  }
  const canActivate = paid && entitlementActive && isEofyEntitlementSessionConfigured();
  ...
  return <><Header /><main className="py-14 sm:py-20"><Container className="max-w-3xl">
    ...
    <div className="mt-8 flex flex-wrap gap-3">
      <EofyProActivationForm initialSessionId={!terminalNotice ? sessionId : undefined} initialNotice={notice} />
      <Link href="/eofy-pro" className="inline-flex min-h-12 items-center justify-center bg-navy px-5 py-3 text-sm font-semibold text-white">제품 소개로 돌아가기</Link>
      {notice !== "refunded" && notice !== "review" && <Link href="/eofy-pro/restore" className="inline-flex min-h-12 items-center justify-center border border-navy px-5 py-3 text-sm font-semibold text-navy">이용권 복구</Link>}
