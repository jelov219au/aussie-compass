  ...
  return <>
    ...
    <main>
      <section className="border-b border-navy/15 py-12 sm:py-20"><Container>
        ...
        <div className="mt-10 flex flex-wrap gap-3"><Link href="/tax-return-guide" className="inline-flex min-h-12 items-center justify-center bg-navy px-5 text-sm font-semibold text-white">무료 체크리스트 먼저 사용</Link><span className="inline-flex min-h-12 items-center border border-border bg-white px-5 text-sm font-semibold text-muted" aria-label="EOFY Pack Pro 출시 준비 중">출시 준비 중 · 결제 없음</span></div>
      </Container></section>
      ...
      <section className="bg-navy py-12 text-white sm:py-16"><Container className="grid gap-6 sm:grid-cols-[1fr_auto] sm:items-end"><div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-gold">Privacy first</p><h2 className="mt-3 text-3xl font-semibold tracking-tight">TFN과 영수증 원본은 받지 않는 방향으로.</h2><p className="mt-3 max-w-2xl text-sm leading-7 text-white/65">현재 제품 설계는 민감한 세금 문서를 서버에 업로드하지 않고, 사용자의 기기에서 준비 목록과 요약을 만드는 방식을 우선합니다.</p></div><Link href="/tax-return-guide" className="inline-flex min-h-12 items-center justify-center bg-gold px-5 text-sm font-semibold text-navy hover:bg-white">무료 가이드 열기 →</Link></Container></section>
