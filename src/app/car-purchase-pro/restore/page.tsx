...
export default async function CarPurchaseProRestorePage() {
  ...
  return <><Header /><main className="py-12 sm:py-16"><Container className="max-w-3xl">
    ...
    <p className="mt-5 rounded-xl border border-gold/50 bg-[#f6f3e9] p-4 text-sm leading-7 text-navy">{accessAvailable
      ? "복구 코드로 이용을 연결하거나 현재 브라우저의 연결을 관리하세요. 다른 브라우저·설치형 앱으로 자동 연결되지 않으며 인터넷 연결이 필요합니다."
      : "현재 복구·연결 관리를 사용할 수 없습니다. 이미 구매했다면 다시 결제하지 말고 고객지원으로 문의해 주세요."}</p>
    <CarPurchaseProRecoveryPanel enabled={accessAvailable} />
