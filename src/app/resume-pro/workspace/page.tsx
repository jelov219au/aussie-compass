...
import { ResumeProWorkspaceEntryGuide } from "@/components/tools/ResumeProWorkspaceEntryGuide";
...
export const metadata: Metadata = {
  title: "Resume Pro 작업 공간 | Hoju Compass",
...
export default async function ResumeProWorkspacePage() {
  ...
  return (
    <>
      <BreadcrumbJsonLd items={[{ name: "홈", path: "/" }, { name: "Resume Pro", path: "/resume-pro" }, { name: "작업 공간", path: "/resume-pro/workspace" }]} />
      <Header />
        ...
        <Container>
          ...
          <div className="mt-7 grid gap-7 border-b border-navy/20 pb-9 lg:grid-cols-[1fr_18rem] lg:items-end">
            ...
            <aside className="border-l-2 border-gold pl-5 text-sm leading-6 text-muted">
              ...
              {accessProtected ? "현재 기기에 연결된 활성 이용권을 확인했습니다." : "기능 검증 단계이며 결제와 계정 생성은 진행되지 않습니다."}
            </aside>
          </div>
          <ResumeProWorkspaceEntryGuide accessProtected={accessProtected} />
          <div id="resume-pro-workspace" tabIndex={-1} className="mt-9 scroll-mt-24 focus:outline-none"><ResumeProWorkspace /></div>
          {accessProtected ? <ResumeProAccessTools /> : <ResumeProDevicePrivacyTools requireRecoveryAcknowledgement={false} />}
