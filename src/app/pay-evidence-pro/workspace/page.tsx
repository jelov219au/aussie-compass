...
import { PayEvidenceProAccessTools } from "@/components/tools/PayEvidenceProAccessTools";
...
export const metadata: Metadata = {
  title: "Pay Evidence Pack Pro 작업 공간 | Hoju Compass",
  description: "근무시간, Payslip Gross·Net, 실제 입금과 증빙 상태를 급여기간별로 정리하고 영문 급여 확인 요청문을 만드세요.",
...
export default async function PayEvidenceWorkspacePage() {
  ...
  if (accessProtected && !await getActivePayEvidenceProEntitlement()) {
    redirect("/pay-evidence-pro?access=required");
  }

  return <>
    <BreadcrumbJsonLd items={[{ name: "홈", path: "/" }, { name: "Pay Evidence Pack Pro", path: "/pay-evidence-pro" }, { name: "작업 공간", path: "/pay-evidence-pro/workspace" }]} />
    <Header />
    <main className="py-12 sm:py-16">
      <Container>
        ...
        <div className="mt-9"><PayEvidenceWorkspace /></div>
        {accessProtected && <PayEvidenceProAccessTools />}
        <section className="mt-10 border border-amber-300 bg-amber-50 p-5 text-sm leading-7 text-amber-950">
          <h2 className="font-semibold">고용·급여 안내</h2>
          <p className="mt-1">Gross 비교는 사용자가 입력한 기대 Gross와 Payslip Gross의 단순 차이이며 법적 미지급액 판정이 아닙니다. Net 대조는 Payslip Net과 실제 입금 Net의 기록 차이만 보여줍니다. Award, Agreement, Classification, penalty·overtime·allowance 적용과 세금·Super는 Fair Work 공식 도구 또는 전문가에게 확인하세요.</p>
        </section>
      </Container>
    </main>
    <Footer />
