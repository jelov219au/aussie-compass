...
import { carBuyProProduct, carBuyProPurchaseTermsVersion, eofyProProduct, eofyProPurchaseTermsVersion, payEvidenceProduct, payEvidencePurchaseTermsVersion, rentalProProduct, rentalProPurchaseTermsVersion, resumeProProduct, resumeProPurchaseTermsVersion } from "@/lib/commerce";
        ...
        <Container className="max-w-6xl">
          ...
          <div className="mt-8 grid gap-8 border-b border-navy/20 pb-10 lg:grid-cols-[1fr_18rem] lg:items-end">
            ...
            <aside className="border-l-2 border-gold pl-5 text-sm leading-6 text-muted">
              ...
              <span className="mt-1 block">Resume {resumeProPurchaseTermsVersion}</span><span className="block">Pay {payEvidencePurchaseTermsVersion}</span><span className="block">Rental {rentalProPurchaseTermsVersion}</span><span className="block">Car Buy {carBuyProPurchaseTermsVersion}</span><span className="block">EOFY {eofyProPurchaseTermsVersion}</span>
              <span className="mt-2 block text-xs">결제할 때 확인한 버전이 구매 기록에 함께 남습니다.</span>
            </aside>
          </div>

          <section className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-3" aria-label="핵심 이용 조건">
            ...
            <article className="border-t-2 border-navy bg-white p-5"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-muted">Car Buy Pack Pro</p><h2 className="mt-3 text-xl font-semibold text-navy">{carBuyPrice} AUD 1회 결제</h2><p className="mt-2 text-sm leading-6 text-muted">중고차 첫해 비용과 구매 전 확인을 정리하는 브라우저 기반 도구이며 차량 상태나 적정 가격을 판정하지 않습니다.</p></article>
            <article className="border-t-2 border-navy bg-white p-5"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-muted">EOFY Pack Pro</p><h2 className="mt-3 text-xl font-semibold text-navy">{eofyPrice} AUD 1회 결제</h2><p className="mt-2 text-sm leading-6 text-muted">소득 자료와 공제 후보, 확인 질문을 정리하는 브라우저 기반 도구이며 신고·공제 판단이나 환급액 계산을 제공하지 않습니다.</p></article>
          ...
          <div className="mt-12 divide-y divide-border border-y border-border">
            ...
            <section className="grid gap-5 py-8 lg:grid-cols-[15rem_1fr]">
              ...
              <div className="max-w-3xl space-y-3 text-sm leading-7 text-muted">
                ...
                <p>Car Buy Pack Pro는 사용자가 입력한 중고차 후보의 첫해 예상 비용, 구매 전 확인 상태, 판매자 질문과 결정 요약을 정리하는 브라우저 기반 디지털 작업 공간입니다. 차량 가치 평가, 기계 검사, 금융·보험 추천 또는 법률 자문을 제공하지 않습니다.</p>
                <p>EOFY Pack Pro는 사용자가 확인한 소득 자료 상태, 공제 후보의 증빙 메모와 세무사에게 물어볼 질문을 한 회계연도 준비 요약으로 정리합니다. 신고 대행, 공제 가능 여부 판정, 예상 환급액 계산 또는 세무 자문을 제공하지 않습니다.</p>
            ...
            <section className="grid gap-5 py-8 lg:grid-cols-[15rem_1fr]">
              ...
              <div className="max-w-3xl space-y-3 text-sm leading-7 text-muted">
                <p>Resume Pro는 {resumePrice} AUD, Pay Evidence Pro와 EOFY Pack Pro는 각각 {payPrice} AUD와 {eofyPrice} AUD, Rental Application Pack Pro와 Car Buy Pack Pro는 각각 {rentalPrice} AUD와 {carBuyPrice} AUD의 1회 결제 상품입니다. 결제 전에 Stripe 화면에서 구매 상품, 최종 금액, 결제수단과 인보이스 정보를 확인할 수 있으며 Hoju Compass는 전체 카드번호나 CVC를 직접 받지 않습니다.</p>
            ...
            <section className="grid gap-5 py-8 lg:grid-cols-[15rem_1fr]">
              ...
              <div className="max-w-3xl space-y-3 text-sm leading-7 text-muted">
                <p>이력서·커버레터·근무시간·급여 계산·Rental Pack의 프로필·집 후보·영문 문구, Car Buy Pack의 차량 별칭·비용·확인 메모와 EOFY Pack의 준비 목록·공제 후보·질문은 별도 안내가 없는 한 현재 브라우저에서 처리됩니다. 결제 이용권 데이터베이스에는 작업 공간의 문서 원문을 저장하지 않습니다.</p>
