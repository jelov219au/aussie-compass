...
const groups: ProjectGroup[] = [
  ...
  { title: "첫 2주", items: [
    ...
    { id: "gp", label: "GP·약국·응급 경로와 결과 확인 방식을 저장", detail: "진료비, 예약 길이, 취소비와 통역을 확인하고 검사 결과를 누가 언제 알려주는지 물으세요. 위급할 때는 000을 이용하세요." },
...
export default function ArrivalChecklistPage() { return <><BreadcrumbJsonLd items={[{ name: "홈", path: "/" }, { name: "호주 도착 후 30일", path: "/arrival-checklist" }]} /><Header /><main className="py-12 sm:py-16"><Container>
  ...
  <section className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3"><Link href="/english-phrase-cards" className="rounded-2xl bg-navy p-6 text-white"><span className="text-sm font-semibold text-gold">말이 막힐 때</span><strong className="mt-2 block text-lg">생활 영어 문장 골라두기 &rarr;</strong><span className="mt-2 block text-sm leading-6 text-white/70">은행·집·직장·병원에서 바로 쓰는 확인 문장</span></Link><Link href="/resources/tfn-application-after-arrival-australia" className="rounded-2xl border border-gold/40 bg-gold/5 p-6"><span className="text-sm font-semibold text-gold">첫 7일</span><strong className="mt-2 block text-lg text-navy">TFN 상세 신청 가이드 &rarr;</strong><span className="mt-2 block text-sm leading-6 text-muted">신청 자격, 28일 처리 흐름과 번호 보안</span></Link><Link href="/resources/australia-sim-esim-setup-guide" className="rounded-2xl border border-border bg-white p-6"><span className="text-sm font-semibold text-gold">도착 당일</span><strong className="mt-2 block text-lg text-navy">SIM·eSIM 안전하게 개통하기 &rarr;</strong><span className="mt-2 block text-sm leading-6 text-muted">요금제, 본인 확인, 번호 이동과 SIM swap 예방</span></Link><Link href="/resources/australia-bank-account-opening-guide" className="rounded-2xl border border-gold/40 bg-gold/5 p-6"><span className="text-sm font-semibold text-gold">첫 계좌 · 첫 명세서</span><strong className="mt-2 block text-lg text-navy">은행 계좌와 자동출금 확인하기 &rarr;</strong><span className="mt-2 block text-sm leading-6 text-muted">수수료·카드·급여·PayID부터 첫 명세서, 사기 대응과 AFCA 민원까지</span></Link><Link href="/public-transport-guide" className="rounded-2xl border border-border bg-white p-6"><span className="text-sm font-semibold text-gold">집과 통학</span><strong className="mt-2 block text-lg text-navy">생활권 후보 비교하기 &rarr;</strong><span className="mt-2 block text-sm leading-6 text-muted">주거비와 학교·직장까지 대중교통 시간 비교</span></Link><Link href="/salary-calculator" className="rounded-2xl border border-border bg-white p-6"><span className="text-sm font-semibold text-gold">첫 직장</span><strong className="mt-2 block text-lg text-navy">예상 급여 확인하기 &rarr;</strong><span className="mt-2 block text-sm leading-6 text-muted">세전·세후 급여와 Super를 함께 계산</span></Link><Link href="/resources/australia-gp-hospital-pharmacy-guide" className="rounded-2xl border border-gold/40 bg-gold/5 p-6"><span className="text-sm font-semibold text-gold">첫 진료 · 첫 청구</span><strong className="mt-2 block text-lg text-navy">예약부터 결과·민원까지 확인하기 &rarr;</strong><span className="mt-2 block text-sm leading-6 text-muted">000·진료비·Referral·검사 결과·Medicare·OSHC 청구와 주·준주 공식 민원</span></Link><Link href="/resources/australia-workplace-injury-workers-compensation-guide" className="rounded-2xl border border-rose-200 bg-rose-50 p-6"><span className="text-sm font-semibold text-rose-700">직장에서 다쳤다면</span><strong className="mt-2 block text-lg text-navy">치료부터 Workers compensation Claim까지 &rarr;</strong><span className="mt-2 block text-sm leading-6 text-muted">응급조치·고용주 통지·Certificate·주별 Claim과 복귀 계획</span></Link></section>
