...
export default function ArrivalChecklistPage() { return <><BreadcrumbJsonLd items={[{ name: "홈", path: "/" }, { name: "호주 도착 후 30일", path: "/arrival-checklist" }]} /><Header /><main className="py-12 sm:py-16"><Container>
  ...
  <section className="mt-10 grid gap-5 lg:grid-cols-3"><Link href="/public-transport-guide" className="rounded-2xl bg-navy p-6 text-white"><span className="text-sm font-semibold text-gold">집과 통학</span><strong className="mt-2 block text-lg">생활권 후보 비교하기 &rarr;</strong><span className="mt-2 block text-sm leading-6 text-white/70">주거비와 학교·직장까지 대중교통 시간 비교</span></Link><Link href="/salary-calculator" className="rounded-2xl border border-border bg-white p-6"><span className="text-sm font-semibold text-gold">첫 직장</span><strong className="mt-2 block text-lg text-navy">예상 급여 확인하기 &rarr;</strong><span className="mt-2 block text-sm leading-6 text-muted">세전·세후 급여와 Super를 함께 계산</span></Link><Link href="/cost-of-living-calculator" className="rounded-2xl border border-border bg-white p-6"><span className="text-sm font-semibold text-gold">첫 달 점검</span><strong className="mt-2 block text-lg text-navy">실제 생활비 기록하기 &rarr;</strong><span className="mt-2 block text-sm leading-6 text-muted">서로 다른 결제 주기를 주·월 단위로 비교</span></Link></section>
