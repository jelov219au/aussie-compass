...
export default function PropertyInspectionChecklistPage() { return <><BreadcrumbJsonLd items={[{ name: "홈", path: "/" }, { name: "집 방문 체크리스트", path: "/property-inspection-checklist" }]} /><Header /><main className="py-12 sm:py-16"><Container>
  ...
  <section className="mt-8 border-y border-navy/20 bg-white p-6 sm:flex sm:items-center sm:justify-between sm:gap-6 sm:p-8"><div><p className="text-xs font-semibold text-gold">다음 준비가 필요할 때 · Pro 유료 검증</p><h2 className="mt-2 text-xl font-semibold text-navy">지원할 집을 정했다면 서류 준비도 이어서 해볼 수 있어요</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-muted">원본 서류를 올리지 않고도 준비할 항목과 개인정보 주의점, 집주인에게 보낼 영문 소개문을 한곳에서 정리할 수 있어요.</p></div><Link href="/rental-application-pro?from=property-inspection-checklist" className="mt-4 inline-flex min-h-11 shrink-0 items-center bg-navy px-4 text-sm font-semibold text-white sm:mt-0">Rental Pack 살펴보기 &rarr;</Link></section>
