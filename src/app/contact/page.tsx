...
import { ContactSupportNextAction } from "@/components/contact/ContactSupportNextAction";
...
const contactTypes = [
  {
    ...
    description: "페이지 주소, 확인한 문장과 확인 시각을 알려주세요. 가능하다면 최신 공식 출처도 함께 보내주세요. 스크린샷 원본이나 개인정보는 보내지 않아도 됩니다.",
    subject: "콘텐츠 정정 요청",
    prompts: ["문의한 페이지 주소:", "확인이 필요한 문장이나 내용:", "오류를 확인한 시각:", "확인한 공식 출처(있다면):", "공식 출처 화면의 갱신일·라이선스 표시(있다면):"],
  },
  {
    eyebrow: "도구 이용",
    ...
    <>
      ...
      <main className="py-12 sm:py-16">
        <Container className="max-w-6xl">
          ...
          <p className="mt-6 text-xs font-semibold uppercase tracking-[0.2em] text-gold-ink">Contact Hoju Compass</p>
          <h1 className="mt-3 max-w-4xl text-4xl font-semibold tracking-tight text-navy sm:text-5xl">문의 유형을 고르면 보낼 곳과 다음 행동을 알려드립니다.</h1>
          <ContactSupportNextAction supportEmail={seller.email} />

          <section className="mt-12" aria-labelledby="contact-type-heading">
            <div className="flex items-end justify-between border-b border-navy/20 pb-4">
              <div><p className="text-xs font-semibold text-gold-ink">문의 내용별 템플릿</p><h2 id="contact-type-heading" className="mt-1 text-2xl font-semibold text-navy">더 자세한 이메일 초안도 준비할 수 있습니다.</h2></div>
            ...
            <ul className="grid gap-px bg-border lg:grid-cols-3">
              {contactTypes.map((item, index) => (
                <li key={item.subject} className="flex flex-col bg-white p-6 sm:p-7">
                  <span className="font-mono text-xs font-semibold text-gold-ink">{String(index + 1).padStart(2, "0")} · {item.eyebrow}</span>
          ...
          <section className="mt-10 grid gap-6 lg:grid-cols-[1fr_0.9fr]">
            <div className="bg-navy p-6 text-white sm:p-8">
              ...
              <p className="mt-6 border-t border-white/15 pt-5 text-sm leading-6 text-white/65">스크린샷을 첨부한다면 이름, 주소, 이메일, 전체 결제 참조와 문서 내용을 가린 뒤 보냅니다. 원본 파일은 필요하지 않습니다.</p>
            </div>
            <div className="border border-border bg-surface p-6 sm:p-8">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-gold-ink">알아두세요</p>
