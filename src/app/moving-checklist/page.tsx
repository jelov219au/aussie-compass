...
const movingGroups:ProjectGroup[]=[
 {title:"4주 전",items:[
  {id:"notice",label:"퇴거 통지와 계약 종료일 확인",detail:"이사일을 확정하기 전에 계약 형태와 주·준주 규정에 맞는 공식 통지 방법·기한을 확인하세요. 4주 전·2주 전 구분은 준비 순서 예시이며 법정 통지 기한이 아닙니다."},
 ...
 {title:"2주 전",items:[
  ...
  {id:"internet",label:"인터넷 이전 예약",detail:"새 주소의 NBN·대체망, 설치 방문, 기존 서비스 종료일과 장비 구분을 확인하세요."},
...
<section className="mt-12 rounded-2xl bg-navy p-6 text-white sm:p-8" aria-labelledby="moving-utility-heading"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-gold">03 · 서비스 이전</p><h2 id="moving-utility-heading" className="mt-2 text-2xl font-semibold">‘주소 변경’과 ‘서비스 이전’은 다릅니다</h2><div className="mt-6 grid gap-6 text-sm leading-7 text-white/75 md:grid-cols-2"><div><h3 className="font-semibold text-white">기존 주소 종료</h3><ul className="mt-3 space-y-2"><li>최종 사용일·계량기 수치·마지막 Bill 주소</li><li>인터넷 종료일·Final bill과 NBN·Provider 장비 구분</li><li>Direct debit 종료와 Credit 환급 계좌</li><li>Hardship·Payment plan과 남은 Debt 처리</li><li>보험 보장 종료시간과 이삿짐 운송 중 보장</li></ul></div><div><h3 className="font-semibold text-white">새 주소 시작</h3><ul className="mt-3 space-y-2"><li>전기·가스 연결일과 Connection fee</li><li>Plan 이름·Tariff·Supply·Usage rate 확인서</li><li>주소별 NBN·대체망·기술과 설치·임시 데이터</li><li>Contents·차량보험 주소와 주차 조건 변경</li><li>건물 엘리베이터·Loading zone 예약 승인</li></ul></div></div><div className="mt-7 grid gap-4 md:grid-cols-2"><article className="rounded-xl border border-white/20 bg-white/10 p-5"><h3 className="font-semibold text-white">첫 Energy Bill까지 확인하세요</h3><p className="mt-2 text-sm leading-7 text-white/75">입주일 Meter 사진, 연결 확인서와 Plan 문서를 보관하고 첫 Bill의 주소·기간·Actual 또는 Estimated reading·Rate를 대조하세요.</p><Link href="/resources/australia-energy-plan-moving-home-guide" className="mt-4 inline-flex min-h-11 items-center border-b-2 border-gold text-sm font-semibold text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold focus-visible:ring-offset-2 focus-visible:ring-offset-navy">전기·가스 연결과 첫 Bill 가이드 →</Link></article><article className="rounded-xl border border-white/20 bg-white/10 p-5"><h3 className="font-semibold text-white">인터넷은 주소 확인부터 시작하세요</h3><p className="mt-2 text-sm leading-7 text-white/75">새 주소의 NBN·대체망과 설치 필요 여부를 확인하고 CIS, Busy period 속도, 연결일·장비·첫 Bill과 Payment assistance를 한 흐름으로 준비하세요.</p><Link href="/resources/australia-home-internet-moving-guide" className="mt-4 inline-flex min-h-11 items-center border-b-2 border-gold text-sm font-semibold text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold focus-visible:ring-offset-2 focus-visible:ring-offset-navy">집 인터넷 연결과 첫 Bill 가이드 →</Link></article></div></section>

<section className="mt-12" aria-labelledby="moving-energy-support-heading"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-gold">04 · Energy 지원</p><h2 id="moving-energy-support-heading" className="mt-2 text-2xl font-semibold text-navy sm:text-3xl">서비스 주소를 고르고 비교·지원·분쟁 경로를 확인하세요</h2><p className="mt-3 max-w-4xl text-sm leading-7 text-muted">아래는 2026년 8월 30일 다시 확인한 정부 비교·지원과 독립 분쟁기관 출발점입니다. Retail market과 고객 보호 체계는 지역마다 다르고, Embedded network나 Remote area는 범위가 달라질 수 있어요.</p><EnergySupportJurisdictionPicker/></section>

<section className="mt-12" aria-labelledby="moving-state-heading"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-gold">05 · 지역 규정</p><h2 id="moving-state-heading" className="mt-2 text-2xl font-semibold text-navy sm:text-3xl">퇴거 통지와 Bond는 주·준주에서 확인하세요</h2><p className="mt-3 max-w-4xl text-sm leading-7 text-muted">아래는 2026년 8월 30일 다시 확인한 정부·공식 임대기관 출발점입니다. 현재 집이 있는 지역을 고르고 계약 형태, 종료 사유, Notice 기한과 공식 양식을 직접 확인하세요.</p><MovingJurisdictionPicker/></section>

<section className="mt-12" aria-labelledby="moving-address-heading"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-gold">06 · 주소 변경</p><h2 id="moving-address-heading" className="mt-2 text-2xl font-semibold text-navy sm:text-3xl">기관마다 바뀌는 정보와 시점이 달라요</h2><div className="mt-6 grid gap-3 md:grid-cols-2">{["운전면허·차량 Rego: 주 교통기관의 변경 기한과 차량 보관주소", "ATO·Medicare·Centrelink: myGov 연결 서비스별 주소·연락처와 주거정보", "은행·카드·Super·보험: Statement·신원확인·위험주소와 우편 수신", "고용주·Payroll: Payslip·PAYG·비상연락 기록", "학교·의료·전문자격: 학생 주소 의무와 예약·기록 전달", "Australia Post 재전송: 보조수단일 뿐 기관 직접 변경을 대신하지 않음"].map((item)=><p key={item} className="flex gap-3 rounded-xl border border-border bg-white p-4 text-sm leading-6 text-muted"><span aria-hidden="true" className="text-gold-ink">□</span><span>{item}</span></p>)}</div><div className="mt-5 flex flex-wrap gap-3"><a href="https://www.servicesaustralia.gov.au/centrelink-online-account-help-update-address-accommodation-and-contact-details" target="_blank" rel="noreferrer" className="inline-flex min-h-11 items-center text-sm font-semibold text-navy underline decoration-gold decoration-2 underline-offset-4">Services Australia 주소 변경 ↗</a><a href="https://www.accc.gov.au/consumers/problem-with-a-product-or-service-you-bought/where-to-go-for-consumer-help" target="_blank" rel="noreferrer" className="inline-flex min-h-11 items-center text-sm font-semibold text-navy underline decoration-gold decoration-2 underline-offset-4">주·준주 Consumer help ↗</a></div></section>
