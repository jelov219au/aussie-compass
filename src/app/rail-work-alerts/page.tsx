...
import { NATIONAL_ROADWORKS_MAP } from "@/lib/railWorkAlerts";
...
export default function RailWorkAlertsPage() {
  return <><BreadcrumbJsonLd items={[{ name: "홈", path: "/" }, { name: "대중교통·생활권 비교", path: "/public-transport-guide" }, { name: "철도 작업 확인", path: "/rail-work-alerts" }]} /><Header /><main className="py-12 sm:py-16"><Container>
    ...
    <section className="mt-8 border-l-4 border-gold bg-surface p-5 sm:p-7" aria-labelledby="rail-journey-check-heading">
      <h2 id="rail-journey-check-heading" className="text-2xl font-semibold text-navy">이번 이동의 날짜와 도착 시각으로 다시 검색하세요.</h2>
      <ol className="mt-4 list-decimal space-y-3 pl-5 text-sm leading-7 text-muted">
        <li>출발역뿐 아니라 환승역·도착역, 실제 이동 날짜와 출발 또는 도착 시각을 공식 안내에 넣습니다.</li>
        <li>전체·부분 구간 변경과 대체버스 승차 위치를 확인하고, 정류장까지 걷는 시간과 환승 여유를 더합니다.</li>
        <li>이동 당일 공식 Journey Planner로 다시 검색하고, 막차·리프트·휠체어 접근성과 환승 시간을 확인합니다. 대체버스 등 일부 구간에는 실시간 정보가 없을 수 있습니다.</li>
      </ol>
      <p className="mt-5 border-t border-border pt-4 text-sm leading-7 text-navy"><strong>가상 예시:</strong> 토요일 08:30 면접이라면 평일 07:45 출발 기록을 그대로 쓰지 마세요. 대체버스 정류장까지의 도보와 환승을 포함해 면접 장소 도착 시각에서 거꾸로 계산하고, 그 토요일의 공식 경로를 확인합니다. 특정 날짜의 실제 운행 중단을 뜻하는 예시는 아닙니다.</p>
    </section>
