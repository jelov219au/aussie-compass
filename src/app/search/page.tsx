...
const coreItems:SearchItem[]=[
  ...
  {href:"/resume-pro",type:"도구",title:"Resume Pro — 공고별 이력서·커버레터",description:"지원할 공고가 정해진 뒤 STAR 경험을 회사별 지원서와 면접 준비에 재사용하고 묶음으로 내보내기",keywords:["이력서","resume","CV","커버레터","cover letter","STAR","STAR examples","면접","인터뷰","selection criteria","유료","pro"]},
...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string | string[] }> }) {
  ...
  return <><BreadcrumbJsonLd items={[{name:"홈",path:"/"},{name:"통합 검색",path:"/search"}]}/><Header/><main className="py-12 sm:py-16"><Container><Link href="/" className="inline-flex min-h-11 items-center text-sm font-medium text-muted hover:text-navy">&larr; 홈으로 돌아가기</Link><div className="mt-8 max-w-4xl"><p className="text-xs font-semibold uppercase tracking-[0.2em] text-gold">막막한 순간에 찾아보세요</p><h1 className="mt-3 text-4xl font-semibold tracking-tight text-navy sm:text-5xl">어떤 정보가 필요한가요?</h1><p className="mt-5 max-w-3xl leading-7 text-muted">도구 이름을 몰라도 괜찮아요. 궁금한 단어나 지금 겪고 있는 상황을 입력하면 관련 도구와 글을 함께 찾아드려요.</p></div><SiteSearch items={items} initialQuery={initialQuery}/><p className="mt-4 border-l-2 border-gold pl-4 text-xs leading-5 text-muted">검색어는 검색 결과 페이지를 여는 데만 사용하며 계정 정보와 연결해 저장하지 않아요. 결과 필터링은 지금 보고 있는 페이지 안에서 처리합니다.</p></Container></main><Footer/></>;
}
