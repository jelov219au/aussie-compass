...
const sections = [
  ["현재 수집하는 개인정보", "Hoju Compass는 현재 회원 계정, 이메일 구독, 문의 양식 또는 광고 프로필을 운영하지 않습니다. 도구에 입력한 이력서·구직·예산·체크리스트 내용은 별도 안내가 없는 한 서버로 제출되지 않습니다. 사용자가 직접 이메일을 보내거나 Resume Pro 결제를 시작한 경우에만 아래 이메일·결제 처리 내용이 적용됩니다."],
  ["이메일 문의", "support@hojucompass.com으로 직접 문의하면 사용자가 보낸 이메일 주소, 이름 또는 표시 이름, 제목, 본문과 첨부파일이 Zoho Mail에서 처리되고 문의 확인과 답변을 위해 보관됩니다. 문의에는 카드번호 전체, CVC, 비밀번호, 인증번호, TFN, 신분증 사본, 비자번호, 건강정보 또는 이력서 원문을 보내지 마세요. 문의 이메일을 마케팅 발송 명단에 자동으로 추가하지 않습니다."],
  ["Stripe 결제와 이용권", "Resume Pro 결제를 시작하면 Stripe Managed Payments 결제 화면으로 이동하며 Stripe가 사용자가 입력한 연락처, 결제와 해당 결제의 세금·인보이스 정보를 처리합니다. 테스트 환경에서는 실제 청구가 일어나지 않습니다. Hoju Compass는 전체 카드번호나 카드 보안번호를 직접 받지 않습니다. 서버에는 Stripe 이벤트·결제 세션·결제수단 관련 식별자, 제품 코드, 확인한 구매 조건의 버전, 이용권 상태와 처리 시각을 저장할 수 있습니다. 작업 공간의 이력서·커버레터 내용은 이 이용권 데이터베이스에 저장하지 않습니다."],
