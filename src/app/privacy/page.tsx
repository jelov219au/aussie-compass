...
export const metadata = createPageMetadata({ title: "데이터와 개인정보 안내 | Aussie Compass", description: "Aussie Compass 도구의 브라우저 저장, 외부 링크와 현재 개인정보 수집 여부를 확인하세요.", path: "/privacy" });

const sections = [
  ["현재 수집하는 개인정보", "Aussie Compass는 현재 회원 계정, 이메일 구독, 문의 양식, 결제 또는 광고 프로필을 운영하지 않습니다. 도구에 입력한 이력서·구직·예산·체크리스트 내용은 별도 안내가 없는 한 서버로 제출되지 않습니다."],
  ...
  ["공유와 이미지 저장", "페이지 공유 버튼은 기기의 기본 공유 메뉴를 열거나 사용자가 선택하면 현재 주소를 클립보드에 복사합니다. SNS 카드 제작 내용과 이미지는 브라우저에서 처리되며 자동으로 외부 SNS에 게시되지 않습니다. PNG 저장과 설명문 복사는 사용자가 버튼을 누를 때만 실행됩니다."],
  ["호스팅 기술 정보", "사이트 제공과 보안을 위해 호스팅·네트워크 제공자가 접속 IP, 브라우저·기기 정보, 요청 시간과 오류 같은 기술 로그를 처리할 수 있습니다. Aussie Compass는 현재 별도의 사용자 행동 분석, 광고 추적 픽셀 또는 맞춤 광고 쿠키를 설치하지 않습니다."],
